...
func TestTinyTerminalSnapshots(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(caseData.name, func(t *testing.T) {
			...
			snapshotPath := filepath.Join("testdata", "tiny_terminal", caseData.name+".txt")
			assertSnapshot(t, snapshotPath, output)
		})
	}
}
...
func assertSnapshot(t *testing.T, path string, rendered string) {
	t.Helper()
