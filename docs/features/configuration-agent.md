...
You can override CLI executable names/paths for runners (`*_bin` fields under `agent`). Cursor is the exception: `agent.cursor_sdk_node_bin` selects the Node.js executable for CueLoop's checked-in Cursor SDK bridge, and the trusted workspace must provide `@cursor/sdk@1.0.11` or `RALPH_CURSOR_SDK_MODULE_PATH` must point to a trusted/global SDK entrypoint.
