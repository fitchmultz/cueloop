...
- **Template-based generation**: Project-type-specific templates with detected facts plus clearly labeled defaults/TODOs
...
cueloop context init --output docs/AGENTS.md

# Interactive mode with guided prompts
cueloop context init --interactive
```
...
CueLoop automatically detects your project type based on files in the repository root:
...
- CI gate: run `cargo fmt --all --check && cargo clippy --workspace --all-targets -- -D warnings && cargo test --workspace` before claiming completion, but verify it against the repo's actual workflow before treating it as the contract.
- Documentation style: preserve the repo's existing file/module documentation style. Add or update file-level docs only when the repo convention, public API surface, or implementation complexity warrants it.
...
Command source: No repo-specific command contract detected; entries below start from common Rust defaults and still need repo verification.

- CI gate: `cargo fmt --all --check && cargo clippy --workspace --all-targets -- -D warnings && cargo test --workspace` — default: common Rust verification suite; verify before treating it as the repo contract.
- Build: `cargo build --workspace` — default: common Rust build command; verify before treating it as the repo contract.
- Test: `cargo test --workspace` — default: common Rust test command; verify before treating it as the repo contract.
...
- CI failing: start with `cargo fmt --all --check && cargo clippy --workspace --all-targets -- -D warnings && cargo test --workspace` and confirm whether the repo documents a stricter local gate.

---
*Generated by CueLoop v1.0.0 - 2026-01-28T12:34:56Z*
*Template version: 1*
```
...
cueloop context update --section troubleshooting --section git-hygiene --file updates.md

# Preview changes without writing
cueloop context update --file updates.md --dry-run
```
...
cueloop context validate --path docs/AGENTS.md
```
...
When `AGENTS.md` is configured in `instruction_files`, CueLoop:
...
CueLoop validates instruction files at config load time:

- File must exist
- Must be valid UTF-8
- Size limit: 1MB per file

Use `cueloop doctor` to check instruction file configuration:
...
1. **Run `cueloop context init` early** in your project to establish conventions
...
CueLoop includes embedded templates for each project type:
...
| `{ci_gate_rule}` | Full non-negotiable CI rule, rendered from detected commands, interactive input, defaults, or TODOs |
| `{documentation_style_rule}` | Documentation-style guidance that preserves existing repo conventions |
| `{command_source_note}` | Short note explaining whether commands were detected, configured interactively, defaulted, or left as TODOs |
| `{ci_command_line}` | CI command line with evidence note |
| `{build_command_line}` | Build command line with evidence note |
| `{test_command_line}` | Test command line with evidence note |
| `{lint_command_line}` | Lint command line with evidence note |
| `{format_command_line}` | Format command line with evidence note |
| `{git_ci_rule}` | Git-hygiene rule tied to the rendered CI command state |
| `{ci_review_expectation}` | Review/PR verification text derived from the rendered CI command state |
...
```bash
cueloop config show --format json | jq '.agent.instruction_files'
```
...
cueloop context init --project-type rust
cueloop context update --file updates.md
```
