# CueLoopMac Settings Window Investigation
Status: Archived
Owner: Maintainers
...
Investigate why opening Settings still creates extra untitled CueLoopMac-owned CGWindows after the app was cut over from a custom AppKit settings presenter to a standard SwiftUI `Settings` scene.
...
- `apps/CueLoopMac/CueLoopMac/CueLoopMacApp.swift`
- `apps/CueLoopMac/CueLoopMac/ASettingsInfra.swift`
- `apps/CueLoopMac/CueLoopMac/SettingsService.swift`
- `apps/CueLoopMac/CueLoopMac/AppDelegate.swift`
- `apps/CueLoopMac/CueLoopMac/AppSettings.swift`
- `apps/CueLoopMac/CueLoopMac/SettingsViewModel.swift`
- `apps/CueLoopMac/CueLoopMac/CueLoopMacCommands.swift`
- `apps/CueLoopMac/CueLoopMac/CueLoopMacApp+URLRouting.swift`

RepoPrompt context-builder analysis was also run against the current CueLoopMac settings/window paths.

## What is no longer the leading theory

- There is not an obvious second active Settings scene in `CueLoopMacApp.swift`.
...
- `AppSettingsCommands` still exists on disk, but it is not currently installed in `CueLoopMacApp.commands`.
...
This is the strongest code-level match for the untitled black `500x500` helper surface. If SwiftUI/AppKit creates any temporary settings host window during construction, CueLoop is currently eligible to drag that window on-screen and keep surfacing it.
...
- rebuilt `CueLoopMac.app` locally with `xcodebuild -project apps/CueLoopMac/CueLoopMac.xcodeproj -scheme CueLoopMac -configuration Debug -destination 'platform=macOS' ... build`
- relaunched the built app from DerivedData
- opened Settings with `Cmd+,`
- inspected live window inventory with `peekaboo list windows --app CueLoopMac --json`
...
- opening Settings still created:
  - one visible `CueLoopMac Settings` window
...
- `peekaboo list windows --app CueLoopMac --json` now shows the normal Settings window and the workspace window only; the prior untitled `500x500` and `312x237` helper surfaces no longer appear
