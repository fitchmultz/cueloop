...
- Foundational helpers such as template, agent-resolution, redaction, ETA, undo, and task-contract modules are broadly reused and still carry the highest maintenance leverage.
...
Scope:
- Decompose the remaining oversized foundational modules (`crates/ralph/src/template/variables.rs`, `crates/ralph/src/agent/resolve.rs`, `crates/ralph/src/redaction.rs`, `crates/ralph/src/eta_calculator.rs`, `crates/ralph/src/undo.rs`, `crates/ralph/src/template/loader.rs`, `crates/ralph/src/contracts/task.rs`, and adjacent shared helpers) into thinner facades plus focused companions.
