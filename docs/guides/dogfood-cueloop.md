...
- runner: `pi`
- requested model: `zai-glm-5.2`
- effective pi model id: `zai/glm-5.2`
- phases: `3`
...
Use `--skip-real-agent` for a fast non-runner check of fixture creation, `cueloop init`, config/doctor/prompt preview, queue/task surfaces, machine queue read, and run dry-run. The `pi`/`zai/glm-5.2` pairing is a disposable dogfood/test-workspace convention for real agent launches; do not change this repository's root `.cueloop/config.jsonc` just to enforce it.
...
- Writes trusted repo config for `pi`, `zai/glm-5.2`, three phases, commit-only publish mode, disabled webhooks, and fixture-local CI.
...
- Runs `cueloop run one --phases 3 --runner pi --model zai/glm-5.2` against the fixture task.
...
One operator-facing friction point was found before the successful run: the requested model string `zai-glm-5.2` is not accepted by the current pi CLI; `pi --list-models` exposes `zai/glm-5.2`. The harness normalizes that known local alias so the default command remains repeatable while the report records both requested and effective model ids.
