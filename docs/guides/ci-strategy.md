...
- When changing `scripts/agent-ci-surface.sh` or path allowlists in `scripts/lib/release_policy.sh`, run or extend `crates/cueloop/tests/agent_ci_surface_contract_test.rs` so representative `noop` / `ci-docs` / `ci-fast` / `ci` / `macos-ci` routing stays pinned.
