...
//!
//! Purpose:
//! - Keep exact tracked `.cueloop`/`.ralph` rejection coverage grouped together.
//!
//! Responsibilities:
...
#[test]
fn pre_public_check_rejects_tracked_exact_cueloop_root_file() {
    let temp_dir = tempfile::tempdir().expect("create temp dir");
    let repo_root = temp_dir.path();

    copy_pre_public_check_fixture(repo_root);
    write_file(&repo_root.join(".cueloop"), "tracked root cueloop file\n");
    ...
    let output = Command::new("bash")
        ...
        .current_dir(repo_root)
        .output()
        .expect("run pre-public-check with tracked exact .cueloop root file");

    assert!(
        !output.status.success(),
        "pre-public-check should reject tracked exact .cueloop root files\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    let combined = format!(
    ...
    assert!(
        combined.contains("Tracked runtime state files outside the public allowlist detected")
            && combined.contains(".cueloop"),
        "tracked exact .cueloop root file rejection should explain the offending path\noutput:\n{}",
        combined
    );
}
...
#[cfg(unix)]
#[test]
fn pre_public_check_rejects_tracked_exact_cueloop_root_symlink() {
    use std::os::unix::fs::symlink;
    ...
    symlink(
        outside_dir.path().join("outside.txt"),
        repo_root.join(".cueloop"),
    )
    .expect("create tracked root .cueloop symlink");
    ...
    let output = Command::new("bash")
        ...
        .current_dir(repo_root)
        .output()
        .expect("run pre-public-check with tracked exact .cueloop root symlink");

    assert!(
        !output.status.success(),
        "pre-public-check should reject tracked exact .cueloop root symlinks\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    let combined = format!(
    ...
    assert!(
        combined.contains("Tracked runtime state files outside the public allowlist detected")
            && combined.contains(".cueloop"),
        "tracked exact .cueloop root symlink rejection should explain the offending path\noutput:\n{}",
        combined
    );
}
