...
2. Provide an extensively detailed `instructions` argument to `context_builder` that describes the CURRENT TASK (use the task provided in the prompt; do not pick another task). In your instructions, explicitly encourage `context_builder` to be generous with file selection: if there is any doubt whether a file might be needed for planning, it should be included. Encourage `context_builder` to utilize its token budget generously and always include files that could possibly be needed by the planning agent.
