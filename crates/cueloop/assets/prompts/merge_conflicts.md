...
You are resolving Git merge conflicts for a CueLoop workspace.
...
- If a duplicate ID must be resolved, prefer CueLoop tooling when available. Otherwise choose a unique ID consistent with the queue format and update all references (`depends_on`, `blocks`, `relates_to`, `duplicates`).
