...
- Formatting: keep using the repo's existing Go formatting workflow; `gofmt` is common, but only enforce what the repo documents.
- Linting: keep using the repo's existing lint flow; `go vet` is a safe default and `golangci-lint` is optional unless the repo already uses it.
...
- Integration tests: use `tests/` or focused `*_test.go` coverage when cross-module behavior is the subject.
- Use table-driven tests when they improve coverage clarity.
- Temp dirs: use `t.TempDir()` for isolated temp directories.
...
- {ci_troubleshooting}
- Dependency issues: check `go.mod` / `go.sum` and keep the repo's documented module workflow intact.
- Build errors: confirm package paths and module layout before changing command assumptions.
