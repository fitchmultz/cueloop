...
fn validate_rejects_duplicate_ids() {
    let queue = QueueFile {
        version: 1,
        tasks: vec![task("CL-0001"), task("CL-0002"), task("CL-0001")],
    };
    let err = validate_queue(&queue, "CL", 4).unwrap_err();
    let msg = format!("{err:#}");
    assert!(
        msg.to_lowercase().contains("duplicate") && msg.contains("CL-0001"),
...
fn validate_allows_missing_request() {
    let mut missing_request = task("CL-0001");
    missing_request.request = None;
    let queue = QueueFile {
        version: 1,
        tasks: vec![missing_request],
    };
    assert!(validate_queue(&queue, "CL", 4).is_ok());
}

#[test]
fn validate_allows_empty_lists() {
    let mut empty_lists = task("CL-0001");
...
fn validate_rejects_missing_created_at() {
    let mut missing_created = task("CL-0001");
...
fn validate_rejects_missing_updated_at() {
    let mut missing_updated = task("CL-0001");
...
fn validate_rejects_invalid_rfc3339() {
    let mut invalid_timestamp = task("CL-0001");
...
fn validate_queue_set_rejects_cross_file_duplicates() {
    ...
    let mut done_task = task_with("CL-0001", TaskStatus::Done, vec!["tag".to_string()]);
    done_task.completed_at = Some("2026-01-18T00:00:00Z".to_string());
    let done = QueueFile {
        version: 1,
        tasks: vec![done_task],
    };
    let err = validate_queue_set(&active, Some(&done), "CL", 4, 10).unwrap_err();
...
fn validate_queue_allows_duplicate_if_one_is_rejected() {
    let mut rejected = task_with("CL-0001", TaskStatus::Rejected, vec!["tag".to_string()]);
    rejected.completed_at = Some("2026-01-18T00:00:00Z".to_string());
    let queue = QueueFile {
        version: 1,
        tasks: vec![
            task_with("CL-0001", TaskStatus::Todo, vec!["tag".to_string()]),
            rejected,
        ],
    };
    assert!(validate_queue(&queue, "CL", 4).is_ok());
...
fn validate_rejects_done_without_completed_at() {
    let mut done_task = task("CL-0001");
...
fn validate_queue_set_allows_duplicate_across_files_if_rejected() {
    ...
    let mut rejected_done = task_with("CL-0001", TaskStatus::Rejected, vec!["tag".to_string()]);
    rejected_done.completed_at = Some("2026-01-18T00:00:00Z".to_string());
    let done = QueueFile {
        version: 1,
        tasks: vec![rejected_done],
    };
    assert!(validate_queue_set(&active, Some(&done), "CL", 4, 10).is_ok());

    let mut rejected_active = task_with("CL-0001", TaskStatus::Rejected, vec!["tag".to_string()]);
...
fn validate_queue_set_rejects_todo_in_done() {
    ...
    let msg = format!("{err:#}");
    assert!(
        msg.contains("done.json") && msg.contains("CL-0001") && msg.contains("Todo"),
        "unexpected error: {msg}"
    );
}
...
fn validate_queue_set_rejects_doing_in_done() {
    ...
    let msg = format!("{err:#}");
    assert!(
        msg.contains("done.json") && msg.contains("CL-0001") && msg.contains("Doing"),
        "unexpected error: {msg}"
    );
}
...
fn validate_queue_set_rejects_draft_in_done() {
    ...
    let msg = format!("{err:#}");
    assert!(
        msg.contains("done.json") && msg.contains("CL-0001") && msg.contains("Draft"),
        "unexpected error: {msg}"
    );
}
...
fn validate_queue_set_allows_terminal_statuses_in_done() {
    ...
    let mut done_task = task_with("CL-0001", TaskStatus::Done, vec!["tag".to_string()]);
    done_task.completed_at = Some("2026-01-18T00:00:00Z".to_string());
    let mut rejected_task = task_with("CL-0003", TaskStatus::Rejected, vec!["tag".to_string()]);
