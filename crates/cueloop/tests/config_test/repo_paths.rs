...
#[test]
fn test_project_config_path_uses_current_runtime_for_json_migration_marker() {
    let dir = TempDir::new().expect("create temp dir");
    let repo_root = dir.path();
    let cueloop_dir = setup_cueloop_dir(&dir);
    fs::write(cueloop_dir.join("config.json"), r#"{"version":2}"#).expect("write old JSON config");
