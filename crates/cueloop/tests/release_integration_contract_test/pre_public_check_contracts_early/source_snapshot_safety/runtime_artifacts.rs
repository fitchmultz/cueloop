...
fn agent_ci_succeeds_outside_git_via_source_snapshot_safety_mode() {
    ...
    write_file(
        &wrapper_makefile,
        "include Makefile\n\n# Test-only stubs so the contract test exercises routing instead of full toolchains.\ntarget/tmp/stamps/cueloop-release-build.stamp:\n\t@mkdir -p target/tmp/stamps\n\t@touch $@\n\t@echo stub-release-stamp\n\ncheck-file-size-limits deps rust-toolchain-check rust-toolchain-drift-check version-check format-check type-check lint test build generate install-verify install macos-preflight macos-build macos-test macos-test-contracts:\n\t@echo stub-$@\n",
    );
