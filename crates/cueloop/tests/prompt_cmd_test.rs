...
//! - Production prompt rendering is exercised only through `cueloop::commands::prompt` public entrypoints.

use anyhow::Result;
use cueloop::cli::scan::ScanMode;
use cueloop::commands::prompt::{
...
fn make_resolved(temp: &TempDir) -> cueloop::config::Resolved {
    ...
    let cfg = Config {
        ...
        agent: AgentConfig {
            ...
            git_publish_mode: Some(cueloop::contracts::GitPublishMode::CommitAndPush),
            ..Default::default()
        },
