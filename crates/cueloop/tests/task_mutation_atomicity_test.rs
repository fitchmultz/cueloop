...
fn find_task<'a>(
    tasks: &'a [cueloop::contracts::Task],
    id: &str,
) -> Option<&'a cueloop::contracts::Task> {
...
fn task_mutate_applies_multi_field_edit_atomically() -> Result<()> {
    ...
    assert_eq!(task.priority, cueloop::contracts::TaskPriority::High);
...
fn task_mutate_conflict_leaves_task_unchanged() -> Result<()> {
    ...
    assert_eq!(task.priority, cueloop::contracts::TaskPriority::Medium);
