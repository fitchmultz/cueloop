...
# - Print the primary binary path and optionally copy primary binary into an app bundle destination.
# Scope:
# - CLI binary preparation only; Xcode and Makefile invoke this as the single bundling entrypoint.
# Usage:
# - scripts/cueloop-cli-bundle.sh --configuration Release --print-path
# - scripts/cueloop-cli-bundle.sh --configuration Debug --bundle-dir /path/to/Contents/MacOS
# - scripts/cueloop-cli-bundle.sh --configuration Release --target x86_64-unknown-linux-gnu --jobs 4 --print-path
# Invariants/assumptions:
# - Cargo and the CueLoop workspace are available locally.
# - The output executable is `cueloop`.
...
usage() {
    cat <<'EOF'
Usage:
  scripts/cueloop-cli-bundle.sh --configuration Debug|Release [--target TRIPLE] [--jobs N] [--print-path] [--bundle-dir DIR]
...
Options:
  ...
  --bundle-dir     Copy the resolved executable into DIR/cueloop
...
profile_dir="debug"
build_args=(-p cueloop-agent-loop --locked --bin "$PRIMARY_BIN_NAME")
...
if [ -n "$BUNDLE_DIR" ]; then
    ...
    chmod +x "$BUNDLE_DIR/$PRIMARY_BIN_NAME"
fi
