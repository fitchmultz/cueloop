...
release_check_prerequisites() {
    ...
    cueloop_log_step "Checking release prerequisites"

    local tool
    for tool in git cargo gh python3; do
        if ! command -v "$tool" >/dev/null 2>&1; then
            cueloop_log_error "Required tool not found: $tool"
            return 1
        fi
        cueloop_log_success "$tool found"
    done

    if ! gh auth status >/dev/null 2>&1; then
        cueloop_log_error "GitHub CLI is not authenticated"
        echo "  Run: gh auth login" >&2
        return 1
    fi
    cueloop_log_success "GitHub CLI authenticated"
    ...
    if [ "$require_publish_credentials" = "1" ]; then
        ...
        if [ -z "${CARGO_REGISTRY_TOKEN:-}" ] && [ ! -f "$cargo_token_file" ]; then
            cueloop_log_error "crates.io publish credentials not found"
            echo "  Run: cargo login" >&2
            echo "  Or set CARGO_REGISTRY_TOKEN for this release" >&2
            return 1
        fi
        cueloop_log_success "crates.io publish credentials found"
    fi
}
...
release_validate_repo_state() {
    ...
    cueloop_log_step "Validating repository state"
    cd "$REPO_ROOT"

    local current_branch
    current_branch=$(git branch --show-current)
    if [ "$current_branch" != "main" ]; then
        cueloop_log_error "Not on main branch (currently on: $current_branch)"
        return 1
    fi
    cueloop_log_success "On main branch"
    ...
    if [ -n "$dirty_files" ]; then
        if [ "$allow_release_metadata_drift" = "1" ] && release_assert_dirty_paths_allowed "$dirty_files"; then
            cueloop_log_success "Working directory contains only release metadata drift"
        else
            cueloop_log_error "Working directory is not clean"
            echo "$dirty_files" | sed 's/^/  /' >&2
            return 1
        fi
    else
        cueloop_log_success "Working directory is clean"
    fi

    if ! git ls-remote origin >/dev/null 2>&1; then
        cueloop_log_error "Cannot access git remote"
        return 1
    fi
    cueloop_log_success "Git remote is accessible"

    if git rev-parse "v$VERSION" >/dev/null 2>&1; then
        if [ "$allow_existing_tag" = "1" ]; then
            cueloop_log_warn "Local tag v$VERSION already exists; verify mode allows that"
        else
            cueloop_log_error "Local tag v$VERSION already exists"
            echo "  Continue the recorded transaction with: scripts/release.sh reconcile $VERSION" >&2
            return 1
        fi
    else
        cueloop_log_success "Local tag v$VERSION does not exist"
    fi

    if git ls-remote --tags origin "refs/tags/v$VERSION" | grep -q "refs/tags/v$VERSION"; then
        if [ "$allow_existing_tag" = "1" ]; then
            cueloop_log_warn "Remote tag v$VERSION already exists; verify mode allows that"
        else
            cueloop_log_error "Remote tag v$VERSION already exists"
            return 1
        fi
    else
        cueloop_log_success "Remote tag v$VERSION does not exist"
    fi
}

release_verify_plan() {
    cueloop_log_step "Verifying release transaction contract"
    if ! release_validate_changelog_shape "$CHANGELOG"; then
        cueloop_log_error "CHANGELOG.md is missing a release-compatible Unreleased section"
        return 1
    fi
    ...
    preview_repo_url=$(cueloop_get_repo_http_url)
    ...
    if ! grep -q "$VERSION" "$preview_file"; then
        cueloop_log_error "Rendered release notes preview is missing the version marker"
        rm -f "$preview_file" "$preview_changelog" "$preview_checksums"
        return 1
    fi

    rm -f "$preview_file" "$preview_changelog" "$preview_checksums"
    cueloop_log_success "Release transaction contract is valid"
}
