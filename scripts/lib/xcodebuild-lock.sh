#!/usr/bin/env bash
#
# Purpose: Provide shared Xcode build lock helpers for CueLoop's macOS Make targets.
# Responsibilities:
...
cueloop_xcode_build_lock_pid_is_running() {
    local pid="$1"
...
cueloop_xcode_build_lock_mtime_epoch() {
    local path="$1"
...
cueloop_xcode_build_lock_is_older_than() {
    local path="$1"
    local min_age_seconds="$2"
    local mtime_epoch
    local now_epoch

    mtime_epoch="$(cueloop_xcode_build_lock_mtime_epoch "$path")" || return 1
    now_epoch="$(date +%s)"
    [ $((now_epoch - mtime_epoch)) -ge "$min_age_seconds" ]
}

cueloop_xcode_build_lock_is_project_owned() {
    case "$1" in
...
cueloop_xcode_build_lock_is_stale() {
    local lock_dir="$1"
    local owner_file
    local owner_pid
    ...
    owner_pid="$(cueloop_xcode_build_lock_read_field "$owner_file" "pid")"

    if [ -n "$owner_pid" ] && cueloop_xcode_build_lock_pid_is_running "$owner_pid"; then
        return 1
    fi
...
cueloop_write_xcode_build_lock_owner() {
    local lock_dir="$1"
    local label="$2"
    local owner_file
...
cueloop_acquire_xcode_build_lock() {
    local lock_dir="$1"
    local label="$2"
    ...
    if ! cueloop_write_xcode_build_lock_owner "$lock_dir" "$label"; then
        rmdir "$lock_dir" 2>/dev/null || true
        return 1
    fi
}

cueloop_release_xcode_build_lock() {
    local lock_dir="$1"
    local owner_file
