#!/usr/bin/env bash
#
# Purpose: Provide shared shell utilities for CueLoop maintenance scripts.
# Responsibilities:
...
cueloop_sha256_file() {
    ...
    if command -v shasum >/dev/null 2>&1; then
        ...
        cueloop_log_error "No SHA256 checksum tool found (expected shasum or sha256sum)"
        return 1
    fi
}
...
cueloop_resolve_make_cmd() {
    ...
    cueloop_log_error "GNU Make is required (install with 'brew install make' and use gmake)."
    return 1
}

cueloop_get_repo_http_url() {
    local remote_url
    remote_url=$(git -C "$REPO_ROOT" remote get-url origin 2>/dev/null || true)
    if [ -z "$remote_url" ]; then
        cueloop_log_error "Failed to resolve git remote 'origin' URL"
        return 1
    fi
    ...
    case "$remote_url" in
        ...
        git@github.com:*)
            ...
            ;;
        *)
            cueloop_log_error "Unsupported origin remote URL format: $remote_url"
            return 1
            ;;
    esac
}

cueloop_get_rust_host_target() {
    local host
    ...
    cueloop_log_error "Unable to determine rustc host target"
    return 1
}
