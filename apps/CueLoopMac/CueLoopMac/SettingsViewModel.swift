/**
 ...
 - Handle per-workspace config (project .cueloop/config.jsonc).
...
enum SettingsProjectConfigLoader {
    ...
    static func loadSynchronously(from configURL: URL) throws -> SettingsProjectConfigSnapshot {
        ...
        let config = try JSONDecoder().decode(CueLoopConfig.self, from: rawData)
...
final class SettingsViewModel {
    ...
    func loadConfig() async {
        ...
        let configURL = workspace.projectConfigFileURL
            ?? workspace.identityState.workingDirectoryURL.appendingPathComponent(".cueloop/config.jsonc")
        ...
        do {
            ...
            CueLoopLogger.shared.error("Failed to load config: \(error)", category: .config)
        }
    ...
    func saveConfig() async {
        ...
        do {
            // Ensure .cueloop directory exists
            let cueloopDir = configURL.deletingLastPathComponent()
            if !FileManager.default.fileExists(atPath: cueloopDir.path) {
                try FileManager.default.createDirectory(at: cueloopDir, withIntermediateDirectories: true)
            }
            ...
            CueLoopLogger.shared.info("Saved config to \(configURL.path)", category: .config)
        } catch {
            errorMessage = "Failed to save config: \(error.localizedDescription)"
            CueLoopLogger.shared.error("Failed to save config: \(error)", category: .config)
        }
    }
