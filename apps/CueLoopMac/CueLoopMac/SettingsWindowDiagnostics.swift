...
final class SettingsPresentationCoordinator: ObservableObject {
    ...
    nonisolated static let contextDidChangeNotification = Notification.Name("com.mitchfultz.cueloop.settings.contextDidChange")
    ...
    private static func resolveDiagnosticsFileURL() -> URL? {
        guard let rawPath = ProcessInfo.processInfo.environment["CUELOOP_SETTINGS_DIAGNOSTICS_PATH"]?
    ...
    private func refreshDiagnosticsContentForPreparedWorkspace() {
        ...
        let configURL = preparedWorkspace.projectConfigFileURL
            ?? preparedDirectoryURL.appendingPathComponent(".cueloop/config.jsonc")
