/**
 ...
 - Status is one of CueLoopTaskStatus cases.
...
import SwiftUI
import CueLoopCore
// Note: CueLoopCore provides accessibility helpers for task and status types

struct KanbanColumnView: View {
    let status: CueLoopTaskStatus
    let tasks: [CueLoopTask]
    let isTaskBlocked: (CueLoopTask) -> Bool
    let isTaskOverdue: (CueLoopTask) -> Bool
    ...
    private func taskCard(for task: CueLoopTask, index: Int, totalCount: Int) -> some View {
        KanbanCardView(
            task: task,
