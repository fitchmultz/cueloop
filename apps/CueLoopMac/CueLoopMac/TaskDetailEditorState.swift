...
final class TaskDetailEditorState: ObservableObject {
    @Published var draftTask: CueLoopTask
    @Published private(set) var baselineTask: CueLoopTask
    ...
    @Published var conflictedExternalTask: CueLoopTask?
    ...
    func resetForLoadedTask(_ task: CueLoopTask) {
        draftTask = task
        baselineTask = task
    ...
    func synchronizeIfNoLocalChanges(with task: CueLoopTask) {
    ...
    func applyMergedTask(_ task: CueLoopTask) {
        resetForLoadedTask(task)
    ...
    func saveChanges(
        ...
        onTaskUpdated: ((CueLoopTask) -> Void)?,
        force: Bool = false
    ) {
