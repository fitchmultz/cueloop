...
struct TaskExecutionPresetSection: View {
    ...
    private var presetButtons: some View {
        ForEach(CueLoopTaskExecutionPreset.allCases) { preset in
            TaskExecutionPresetButton(
                preset: preset,
                isActive: activeExecutionPreset == preset,
                action: {
                    draftTask.agent = CueLoopTaskAgent.normalizedOverride(preset.agentOverride)
                }
            )
        }
    }
    ...
    private var activeExecutionPreset: CueLoopTaskExecutionPreset? {
        CueLoopTaskExecutionPreset.matchingPreset(for: draftTask.agent)
    }
}
...
struct TaskExecutionSummarySection: View {
    ...
    private var overrideSummaryCaption: String {
        guard let agent = CueLoopTaskAgent.normalizedOverride(draftTask.agent) else {
