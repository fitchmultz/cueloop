...
enum AppColorScheme: String, CaseIterable, Codable, Identifiable {
    ...
    var helperText: String {
        ...
        case .light:
            return "Force CueLoop into light mode."
        case .dark:
            return "Force CueLoop into dark mode."
        }
    }
...
final class AppAppearanceController: ObservableObject {
    ...
    @Published var selection: AppColorScheme {
        didSet {
            ...
            CueLoopAppDefaults.userDefaults.set(selection.rawValue, forKey: Self.colorSchemeKey)
            applyAppearance()
        }
    }
    ...
    private init() {
        let storedValue = CueLoopAppDefaults.userDefaults.string(forKey: Self.colorSchemeKey)
