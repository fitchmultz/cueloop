...
struct TaskExecutionPresetButton: View {
    let preset: CueLoopTaskExecutionPreset
    let isActive: Bool
    let action: () -> Void
...
struct PhaseOverrideEditor: View {
    let title: String
    let phase: Int
    @Binding var draftTask: CueLoopTask
    let workspace: Workspace
    let mutateTaskAgent: ((inout CueLoopTaskAgent) -> Void) -> Void
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 12) {
                ...
                TextField("Model (inherit if empty)", text: phaseModelBinding)
                    ...
                    .help(modelHint ?? "When empty, CueLoop uses the runner's default model resolution.")
    ...
    private var phaseOverride: CueLoopTaskPhaseOverride? {
        switch phase {
    ...
    private func setPhaseOverride(_ value: CueLoopTaskPhaseOverride?) {
        mutateTaskAgent { agent in
            var overrides = agent.phaseOverrides ?? CueLoopTaskPhaseOverrides()
            switch phase {
    ...
    private var phaseRunnerBinding: Binding<String> {
        Binding(
            ...
            set: { value in
                var updated = phaseOverride ?? CueLoopTaskPhaseOverride()
...
struct IgnoredOverridesWarning: View {
    ...
    private func trimIgnoredPhaseOverrides() {
        ...
        agent.phaseOverrides = overrides.isEmpty ? nil : overrides
        draftTask.agent = CueLoopTaskAgent.normalizedOverride(agent)
    }
}
