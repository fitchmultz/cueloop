/**
 ...
 - Intended for use only with `TaskDetailView` in the CueLoopMac app target.
...
extension TaskDetailView {
    func statusColor(_ status: CueLoopTaskStatus) -> Color {
        switch status {
        case .draft:
            return .gray
        case .todo:
    ...
    func priorityColor(_ priority: CueLoopTaskPriority) -> Color {
        switch priority {
        case .critical:
            return .red
        case .high:
...
private struct TaskDetailAlertsModifier: ViewModifier {
    ...
    let draftTask: CueLoopTask
    let conflictedExternalTask: CueLoopTask?
    let onDiscard: () -> Void
    let onForceSave: () -> Void
    let onDiscardExternal: () -> Void
    let onMerge: (CueLoopTask) -> Void
...
extension View {
    ...
    func withTaskDetailAlerts(
        ...
        draftTask: CueLoopTask,
        conflictedExternalTask: CueLoopTask?,
        onDiscard: @escaping () -> Void,
        onForceSave: @escaping () -> Void,
        onDiscardExternal: @escaping () -> Void,
        onMerge: @escaping (CueLoopTask) -> Void
    ) -> some View {
