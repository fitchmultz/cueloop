...
@MainActor
struct CueLoopMacPresentationRuntime {
    static var suppressUserActivation: Bool {
        CueLoopAppDefaults.isMacOSContract
    }
...
final class SettingsSmokeContractRunner {
    ...
    private enum EnvironmentKey {
        static let workspaceA = "CUELOOP_SETTINGS_SMOKE_WORKSPACE_A"
        static let workspaceB = "CUELOOP_SETTINGS_SMOKE_WORKSPACE_B"
        static let reportPath = "CUELOOP_SETTINGS_SMOKE_REPORT_PATH"
    }
    ...
    func configureIfNeeded() {
        guard CueLoopAppDefaults.isSettingsSmokeContract else { return }
    ...
    private func runContract(_ configuration: Configuration) async throws -> SettingsSmokeContractReport {
        ...
        CueLoopURLRouter.handle(workspaceOpenURL(for: configuration.workspaceBURL))
        try await waitForWorkspaceRetarget(path: configuration.workspaceBPath)

        let settingsURL = try settingsRouteURL()
        CueLoopURLRouter.handle(settingsURL)
