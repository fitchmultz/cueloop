...
struct PriorityDistributionCard: View {
    ...
    private var priorityCounts: [(priority: CueLoopTaskPriority, count: Int)] {
        let grouped = Dictionary(grouping: tasks) { $0.priority }
        return CueLoopTaskPriority.allCases.map { priority in
    ...
    private func priorityDistributionAccessibilityText(data: [(priority: CueLoopTaskPriority, count: Int)]) -> String {
