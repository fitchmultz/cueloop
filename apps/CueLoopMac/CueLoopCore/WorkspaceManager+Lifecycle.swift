/**
 ...
 - Restorable workspaces must still exist on disk and retain enough CueLoop state to resolve their queue path.
...
public extension WorkspaceManager {
    ...
    func workspaceIsRestorable(_ url: URL) -> Bool {
        ...
        let configURL = url.appendingPathComponent(".cueloop/config.jsonc", isDirectory: false)
        if FileManager.default.fileExists(atPath: configURL.path) {
            return true
        }
        let cueloopDirectoryURL = url.appendingPathComponent(".cueloop", isDirectory: true)
        return FileManager.default.fileExists(atPath: cueloopDirectoryURL.path)
    }

    private func workspaceSnapshot(_ workspaceID: UUID) -> CueLoopWorkspaceDefaultsSnapshot? {
        let snapshotKeyPrefix = CueLoopAppDefaults.productionDomainIdentifier + ".workspace."
        do {
    ...
    private func restoredWorkingDirectory(for snapshot: CueLoopWorkspaceDefaultsSnapshot) -> URL {
