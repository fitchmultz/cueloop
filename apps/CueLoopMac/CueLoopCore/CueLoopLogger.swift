...
/// Centralized logging system for CueLoop macOS app
/// Note: OSLog.Logger is thread-safe, so this class is marked as @unchecked Sendable
public final class CueLoopLogger: @unchecked Sendable {
    public static let shared = CueLoopLogger()
    ...
    public static let subsystem = "com.mitchfultz.cueloop"
    ...
    private init() {
        ...
        for category in Category.allCases {
            loggersTemp[category] = Logger(subsystem: CueLoopLogger.subsystem, category: category.rawValue)
        }
        loggers = loggersTemp
    }
    ...
    public func logger(for category: Category) -> Logger {
        return loggers[category] ?? Logger(subsystem: CueLoopLogger.subsystem, category: "Default")
    }
    ...
    public func exportLogs(hours: Int = 24) async throws -> String {
        ...
        let entries = try store.getEntries(at: position)
            ...
            .filter { $0.subsystem == CueLoopLogger.subsystem }
