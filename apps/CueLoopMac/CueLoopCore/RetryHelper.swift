    ...
    public static func defaultShouldRetry(_ error: any Error) -> Bool {
        ...
        if let retryable = error as? RetryableError {
            ...
            case .processError(let exitCode, let stderr):
                ...
                return CueLoopCLITransientErrorPolicy.isRetryableProcessError(
                    exitCode: exitCode,
                    stderr: stderr
                )
            case .underlying(let underlying):
                return CueLoopCLITransientErrorPolicy.isRetryableUnderlyingError(underlying)
            }
        }

        if CueLoopCLITransientErrorPolicy.recoveryCategory(for: error.localizedDescription) != nil {
            return true
        }
        return CueLoopCLITransientErrorPolicy.isRetryableUnderlyingError(error)
    }
}
