...
public actor CLIHealthChecker {
    ...
    public static func isCLIUnavailableError(_ error: any Error) -> Bool {
        if let cliError = error as? CueLoopCLIClientError {
            switch cliError {
    ...
    private func performHealthCheck(
        ...
        do {
            let client = if let executableURL {
                try CueLoopCLIClient(executableURL: executableURL)
            } else {
                try CueLoopCLIClient.bundled()
            }
        ...
        } catch CueLoopCLIClientError.executableNotFound {
            return CLIHealthStatus(
                availability: .unavailable(reason: .cliNotFound),
                lastChecked: Date(),
                workspaceURL: workspaceURL
            )
        } catch CueLoopCLIClientError.executableNotExecutable {
            return CLIHealthStatus(
