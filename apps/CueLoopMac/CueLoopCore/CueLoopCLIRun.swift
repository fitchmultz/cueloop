/**
 ...
 - Instances are created only by `CueLoopCLIClient.start(...)`.
...
final class CueLoopCLIProcessSignalState: @unchecked Sendable {
...
public actor CueLoopCLIRun {
    public let events: AsyncStream<CueLoopCLIEvent>
    ...
    private let processSignalState: CueLoopCLIProcessSignalState
    private let stdoutHandle: FileHandle
    private let stderrHandle: FileHandle

    private var eventsContinuation: AsyncStream<CueLoopCLIEvent>.Continuation?
    ...
    private var exitStatus: CueLoopCLIExitStatus?
    private var exitWaiters: [CheckedContinuation<CueLoopCLIExitStatus, Never>] = []
    private var droppedOutputByteCount = 0

    internal init(
        ioQueue: DispatchQueue,
        process: Process,
        processSignalState: CueLoopCLIProcessSignalState,
        ...
        var continuation: AsyncStream<CueLoopCLIEvent>.Continuation?
        let stream = AsyncStream<CueLoopCLIEvent>(
    ...
    public func waitUntilExit() async -> CueLoopCLIExitStatus {
    ...
    private func handleReadable(stream: CueLoopCLIEvent.Stream, handle: FileHandle) {
    ...
    private func yieldBoundedEvent(stream: CueLoopCLIEvent.Stream, data: Data) {
    ...
    private func yieldEvent(stream: CueLoopCLIEvent.Stream, data: Data) {
        let result = eventsContinuation?.yield(CueLoopCLIEvent(stream: stream, data: data))
    ...
    private func handleTermination(process: Process) {
        ...
        CueLoopLogger.shared.debug("CLI process terminated with status: \(process.terminationStatus)", category: .cli)

        let reason: CueLoopCLIExitStatus.TerminationReason
        ...
        let status = CueLoopCLIExitStatus(code: process.terminationStatus, reason: reason)
    ...
    private func drainRemainingOutput(stream: CueLoopCLIEvent.Stream, handle: FileHandle) {
        while true {
            switch readBoundedChunk(from: handle) {
            case .data(let data):
                yieldBoundedEvent(stream: stream, data: data)
