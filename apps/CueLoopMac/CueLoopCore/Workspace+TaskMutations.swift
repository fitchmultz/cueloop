...
extension Workspace {
  ...
  public func updateTask(
    from original: CueLoopTask, to updated: CueLoopTask, originalUpdatedAt: Date? = nil
  ) async throws {
  ...
  public func updateTaskStatus(taskID: String, to newStatus: CueLoopTaskStatus) async throws {
  ...
  public func bulkUpdateStatus(
    taskIDs: [String], to newStatus: CueLoopTaskStatus, skipReload: Bool = false
  ...
  public func bulkUpdatePriority(
    taskIDs: [String], to newPriority: CueLoopTaskPriority, skipReload: Bool = false
  ) async throws -> [(String, String)] {
    let selectedTasks = try selectedTasks(for: taskIDs)
    let request = WorkspaceTaskMutationRequest(
      tasks: selectedTasks.map { task in
...
extension Workspace {
  fileprivate func taskEdits(from original: CueLoopTask, to updated: CueLoopTask) throws
    -> [WorkspaceTaskFieldEdit]
  {
    ...
    let originalAgent = CueLoopTaskAgent.normalizedOverride(original.agent)
    let updatedAgent = CueLoopTaskAgent.normalizedOverride(updated.agent)
  ...
  fileprivate func executeTaskMutationRequest(
    ...
    do {
      ...
      let report = try CueLoopMachineContract.decode(
  ...
  fileprivate func selectedTasks(for taskIDs: [String]) throws -> [CueLoopTask] {
    let requested = Array(
      NSOrderedSet(
  ...
  fileprivate static func encodeTaskAgentFieldValue(_ agent: CueLoopTaskAgent?) throws -> String {
