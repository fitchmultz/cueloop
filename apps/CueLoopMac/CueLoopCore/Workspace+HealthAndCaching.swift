...
public final class WorkspaceDiagnosticsState: ObservableObject {
    ...
    @Published public var cachedTasks: [CueLoopTask] = []
...
public extension Workspace {
    ...
    func refreshCachedTasks() {
        ...
        do {
            ...
            CueLoopAppDefaults.userDefaults.set(data, forKey: defaultsKey("cachedTasks"))
    ...
    func loadCachedTasks() {
        guard let data = CueLoopAppDefaults.userDefaults.data(forKey: defaultsKey("cachedTasks")) else {
            diagnosticsState.cachedTasks = []
            return
        }

        do {
            diagnosticsState.cachedTasks = try JSONDecoder().decode([CueLoopTask].self, from: data)
            clearPersistenceIssue(domain: .cachedTasks)
        } catch {
            recordPersistenceIssue(
                PersistenceIssue(
    ...
    func clearCachedTasks() {
        diagnosticsState.cachedTasks = []
        CueLoopAppDefaults.userDefaults.removeObject(forKey: defaultsKey("cachedTasks"))
        clearPersistenceIssue(domain: .cachedTasks)
        refreshOperationalHealth()
    }
