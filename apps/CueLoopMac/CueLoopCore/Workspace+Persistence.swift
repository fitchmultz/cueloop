/**
 ...
 - Machine-resolved paths override fallback `.cueloop/...` defaults once available.
 */
...
struct CueLoopWorkspaceDefaultsSnapshot: Codable, Sendable {
    let name: String
...
struct WorkspaceStateStore {
    ...
    init(defaults: UserDefaults = CueLoopAppDefaults.userDefaults) {
        self.defaults = defaults
    }

    func load(id: UUID, keyPrefix: String) throws -> CueLoopWorkspaceDefaultsSnapshot? {
        guard let data = defaults.data(forKey: snapshotKey(id: id, keyPrefix: keyPrefix)) else {
            return nil
        }
        return try JSONDecoder().decode(CueLoopWorkspaceDefaultsSnapshot.self, from: data)
    }

    func save(_ snapshot: CueLoopWorkspaceDefaultsSnapshot, id: UUID, keyPrefix: String) throws {
...
public extension Workspace {
    func defaultsKey(_ suffix: String) -> String {
        "com.mitchfultz.cueloop.workspace.\(id.uuidString).\(suffix)"
    }

    var hasCueLoopQueueFile: Bool {
    ...
    var doneFileURL: URL {
        resolvedDoneFileURL
            ?? identityState.workingDirectoryURL.appendingPathComponent(".cueloop/done.jsonc", isDirectory: false)
    }
    ...
    var projectConfigFileURL: URL? {
        ...
        return identityState.workingDirectoryURL.appendingPathComponent(".cueloop/config.jsonc", isDirectory: false)
    }
    ...
    static func defaultQueueFileURL(in workingDirectoryURL: URL) -> URL {
        workingDirectoryURL.appendingPathComponent(".cueloop/queue.jsonc", isDirectory: false)
    }
    ...
    func loadState() {
        ...
        do {
            guard let snapshot = try store.load(
                id: id,
                keyPrefix: CueLoopAppDefaults.productionDomainIdentifier + ".workspace."
            ) else {
    ...
    func persistState() {
        let store = WorkspaceStateStore()
        let snapshot = CueLoopWorkspaceDefaultsSnapshot(
...
extension Workspace {
    ...
    static func restoreRecentWorkingDirectories(from snapshot: CueLoopWorkspaceDefaultsSnapshot) -> [URL] {
