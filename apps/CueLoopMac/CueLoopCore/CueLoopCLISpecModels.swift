/**
 ...
 - `CueLoopCLISpecDocument.version` changes only for breaking schema updates.
 - `MachineCLISpecDocument.version` versions the app-facing wrapper independently.
 - Unknown raw JSON should remain decodable through `CueLoopCLISpec`.
 */
...
public struct CueLoopCLISpec: Codable, Equatable, Sendable {
    public let raw: CueLoopJSONValue

    public init(raw: CueLoopJSONValue) {
        self.raw = raw
    }

    public init(from decoder: any Decoder) throws {
        self.raw = try CueLoopJSONValue(from: decoder)
    }
...
public struct CueLoopCLISpecDocument: Codable, Equatable, Sendable {
    public let version: Int
    public let root: CueLoopCLICommandSpec

    public init(version: Int, root: CueLoopCLICommandSpec) {
        self.version = version
        self.root = root
    }
}

public extension CueLoopCLISpecDocument {
    static let expectedVersion = 2

    func containsCommandSuffix(_ expected: [String]) -> Bool {
        guard !expected.isEmpty else { return false }

        func walk(_ command: CueLoopCLICommandSpec) -> Bool {
...
public struct MachineCLISpecDocument: Codable, Equatable, Sendable, VersionedMachineDocument {
    public static let expectedVersion = CueLoopMachineContract.cliSpecVersion
    public static let documentName = "machine cli-spec"

    public let version: Int
    public let spec: CueLoopCLISpecDocument
}

public struct CueLoopCLICommandSpec: Codable, Equatable, Sendable, Identifiable, Hashable {
    ...
    public let args: [CueLoopCLIArgSpec]
    public let subcommands: [CueLoopCLICommandSpec]
...
public struct CueLoopCLIArgSpec: Codable, Equatable, Sendable, Identifiable, Hashable {
...
public extension CueLoopCLIArgSpec {
