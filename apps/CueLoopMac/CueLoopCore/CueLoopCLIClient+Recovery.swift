...
//! - Delegate classification to `CueLoopCLIRecoveryClassifier` so every path shares one rule set.
//!
//! Scope:
...
public extension RecoveryError {
    static func classify(error: any Error, operation: String, workspaceURL: URL? = nil) -> RecoveryError {
        CueLoopCLIRecoveryClassifier.classify(
            error: error,
            operation: operation,
