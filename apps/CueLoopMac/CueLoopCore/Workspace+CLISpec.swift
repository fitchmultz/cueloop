...
public final class WorkspaceCommandState: ObservableObject {
    @Published public var cliSpec: CueLoopCLISpecDocument?
...
public extension Workspace {
    ...
    func selectedAdvancedCommand() -> CueLoopCLICommandSpec? {
    ...
    func buildAdvancedArguments() -> [String] {
        ...
        var selections: [String: CueLoopCLIArgValue] = [:]
        ...
        return CueLoopCLIArgumentBuilder.buildArguments(
            command: cmd,
...
private extension Workspace {
    nonisolated static let supportedMachineCLISpecDocumentVersion = CueLoopMachineContract.cliSpecVersion
    nonisolated static let supportedCLISpecVersion = CueLoopCLISpecDocument.expectedVersion
    ...
    nonisolated static func validateMachineCLISpecVersion(_ document: MachineCLISpecDocument) throws {
        guard document.version == supportedMachineCLISpecDocumentVersion else {
            throw NSError(
                domain: "CueLoopMachineContract",
                code: 3,
                userInfo: [
                    NSLocalizedDescriptionKey:
                        "Unsupported machine cli-spec version \(document.version). CueLoopMac requires version \(supportedMachineCLISpecDocumentVersion)."
                ]
            )
        }
        ...
        guard document.spec.version == supportedCLISpecVersion else {
            throw NSError(
                ...
                userInfo: [
                    NSLocalizedDescriptionKey:
                        "Unsupported cli-spec schema version \(document.spec.version). CueLoopMac requires version \(supportedCLISpecVersion)."
                ]
            )
        }
    }
    ...
    func collectCommands(
        _ command: CueLoopCLICommandSpec,
        includeHidden: Bool,
        into out: inout [CueLoopCLICommandSpec]
    ) {
