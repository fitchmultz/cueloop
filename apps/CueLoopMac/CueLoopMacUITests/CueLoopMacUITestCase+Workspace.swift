/**
 Purpose:
 - Own CLI-backed workspace and process helpers for CueLoop macOS UI tests.
 ...
 - `cueloopExecutableURL` points to the primary bundled CueLoop CLI before command helpers run.
...
@MainActor
extension CueLoopMacUITestCase {
    func makeUITestWorkspace() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("cueloop-ui-tests", isDirectory: true)
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)

        try runCueLoop(arguments: ["init", "--non-interactive"], currentDirectoryURL: root)
    ...
    func runCueLoop(arguments: [String], currentDirectoryURL: URL) throws {
        _ = try runCueLoopAndCollectOutput(arguments: arguments, currentDirectoryURL: currentDirectoryURL)
    }

    func runCueLoopAndCollectOutput(arguments: [String], currentDirectoryURL: URL) throws -> String {
        guard let executableURL = cueloopExecutableURL else {
            throw NSError(
                domain: "CueLoopMacUITests",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Failed to resolve a cueloop executable for UI tests"]
            )
        }
    ...
    func uiTestWorkspaceTasks() throws -> [UITaskSnapshot] {
        ...
        let output = try runCueLoopAndCollectOutput(
    ...
    func resolveCueLoopExecutableURL(
        environment: [String: String] = ProcessInfo.processInfo.environment
    ) throws -> URL {
        if let override = cueloopExecutableOverride(from: environment), !override.isEmpty {
        ...
        let primaryURL = executableDirectory
            .appendingPathComponent("cueloop", isDirectory: false)
            .standardizedFileURL
            .resolvingSymlinksInPath()
        if FileManager.default.isExecutableFile(atPath: primaryURL.path) {
            return primaryURL
        }
    ...
    private func cueloopExecutableOverride(from environment: [String: String]) -> String? {
        ...
        return environment[LaunchEnvironment.cueloopBinPath]?.trimmingCharacters(in: .whitespacesAndNewlines)
    }
    ...
    func openWorkspaceURLInApp(_ workspaceURL: URL) throws {
        DistributedNotificationCenter.default().postNotificationName(
            Notification.Name("com.mitchfultz.cueloop.uitesting.openWorkspace"),
            object: nil,
