/**
 Purpose:
 - Define the shared CueLoop macOS UI-test base case and its core state.
 ...
 Usage:
 - Subclass `CueLoopMacUITestCase` from focused UI suites.

 Invariants/Assumptions:
 - UI tests launch CueLoop with `--uitesting`.
...
@MainActor
class CueLoopMacUITestCase: XCTestCase {
    ...
    enum ScreenshotCaptureMode: Equatable {
        ...
        static func fromEnvironment(_ environment: [String: String]) -> Self {
            if let rawMode = environment["CUELOOP_UI_SCREENSHOT_MODE"]?
            ...
            if environment["CUELOOP_UI_SCREENSHOTS"] == "1" {
                return .checkpoints
            }
    ...
    enum LaunchEnvironment {
        static let uiTestWorkspacePath = "CUELOOP_UI_TEST_WORKSPACE_PATH"
    ...
    override func setUp() async throws {
        ...
        app = XCUIApplication()
        cueloopExecutableURL = try resolveCueLoopExecutableURL()
        ...
        if let cueloopExecutableURL {
            app.launchEnvironment[LaunchEnvironment.cueloopBinPath] = cueloopExecutableURL.path
        }
