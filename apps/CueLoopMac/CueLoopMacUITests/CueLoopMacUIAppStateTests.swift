/**
 ...
 Usage:
 - Runs against the isolated UI-test workspace created by `CueLoopMacUITestCase`.
...
@MainActor
final class CueLoopMacUIAppStateTests: CueLoopMacUITestCase {
    ...
    func test_urlOpenBootstrapWorkspaceImmediatelyShowsNewWorkspaceQueue() throws {
        ...
        if let cueloopExecutableURL {
            relaunchedApp.launchEnvironment[LaunchEnvironment.cueloopBinPath] = cueloopExecutableURL.path
        }
        app = relaunchedApp
        app.launch()
        app.activate()
