/**
 WorkspaceQueueRefreshWatcherTests

 Purpose:
 - Validate watcher-driven queue refresh and workspace retarget refresh behavior.

 Responsibilities:
 - Cover atomic queue replacement, workspace directory retargeting, and watcher retargeting when queue config paths change.
 - Preserve queue graph and analytics refresh assertions for external file changes.

 Does not handle:
 - Workspace-overview unsupported fallback and error-classification scenarios.
 - Foundational initial queue hydration tests.
...
extension WorkspaceQueueRefreshTests {
    func test_workspaceWatcherAtomicQueueReplacement_refreshesQueueGraphAndAnalytics() async throws {
        ...
        let workspaceURL = try WorkspaceTaskCreationTestSupport.makeTempDir(prefix: "cueloop-workspace-refresh-replace-")
        ...
        let queueRefreshed = await CueLoopCoreTestSupport.waitUntil(timeout: .seconds(10)) {
            await MainActor.run {
        ...
        let derivedViewsRefreshed = await CueLoopCoreTestSupport.waitUntil(timeout: .seconds(10)) {
            await MainActor.run {
    ...
    func test_workspaceRetarget_refreshesQueueWithoutEagerGraphOrAnalyticsForNewDirectory() async throws {
        ...
        var workspace: Workspace!
        let emptyWorkspaceURL = try WorkspaceTaskCreationTestSupport.makeTempDir(prefix: "cueloop-workspace-retarget-empty-")
        let populatedWorkspaceURL = try WorkspaceTaskCreationTestSupport.makeTempDir(prefix: "cueloop-workspace-retarget-populated-")
        defer {
            CueLoopCoreTestSupport.shutdownAndRemove(emptyWorkspaceURL, workspace)
            CueLoopCoreTestSupport.shutdownAndRemove(populatedWorkspaceURL, populatedWorkspace)
        }
        ...
        let loadedInitialState = await CueLoopCoreTestSupport.waitUntil(timeout: .seconds(10)) {
            await MainActor.run {
                workspace.taskState.tasks.isEmpty
                    && workspace.insightsState.graphData == nil
                    && workspace.insightsState.analytics.lastRefreshedAt == nil
        ...
        let retargeted = await CueLoopCoreTestSupport.waitUntil(timeout: .seconds(10)) {
            await MainActor.run {
    ...
    func test_workspaceOverviewWatcherRetargetsWhenConfigMovesQueuePath() async throws {
        var workspace: Workspace!
        let rootURL = try WorkspaceTaskCreationTestSupport.makeTempDir(prefix: "cueloop-workspace-overview-path-switch-")
        ...
        try FileManager.default.createDirectory(at: workspaceURL.appendingPathComponent(".cueloop", isDirectory: true), withIntermediateDirectories: true)

        let initialTask = CueLoopMockCLITestSupport.task(id: "RQ-OVERVIEW-1", status: .todo, title: "Initial overview task", priority: .medium, createdAt: "2026-04-25T00:00:00Z", updatedAt: "2026-04-25T00:00:00Z")
        let switchedTask = CueLoopMockCLITestSupport.task(id: "RQ-OVERVIEW-2", status: .todo, title: "Switched queue task", priority: .high, createdAt: "2026-04-25T01:00:00Z", updatedAt: "2026-04-25T01:00:00Z")
        let defaultQueueURL = workspaceURL.appendingPathComponent(".cueloop/queue.jsonc", isDirectory: false)
        let defaultDoneURL = workspaceURL.appendingPathComponent(".cueloop/done.jsonc", isDirectory: false)
        ...
        let switchedPaths = CueLoopMockCLITestSupport.MockResolvedPathOverrides(queueURL: customQueueURL, doneURL: customDoneURL, projectConfigURL: projectConfigURL)
        ...
        try FileManager.default.copyItem(at: try WorkspaceRunnerConfigurationTestSupport.writeGraphDocument(in: rootURL, name: "graph-switched.json", tasks: [CueLoopMockCLITestSupport.graphNode(id: switchedTask.id, title: switchedTask.title)]), to: graphCurrentURL)
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(in: rootURL, name: "mock-cueloop-overview-path-switch", body: """
            #!/bin/sh
            set -eu
            ...
            exit 64
            """)
        workspace = Workspace(workingDirectoryURL: workspaceURL, client: try CueLoopCLIClient(executableURL: scriptURL), bootstrapRepositoryStateOnInit: false)

        await workspace.refreshWorkspaceOverviewState(retryConfiguration: .minimal)
        let watcherStarted = await CueLoopCoreTestSupport.waitUntil(timeout: .seconds(10)) {
        ...
        let switched = await CueLoopCoreTestSupport.waitUntil(timeout: .seconds(10)) {
            await MainActor.run {
