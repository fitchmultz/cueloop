...
final class WindowStatePersistenceTests: WindowStateTestCase {
    ...
    func test_createWorkspace_persistsInitialWorkingDirectory() throws {
        ...
        let snapshot = try JSONDecoder().decode(CueLoopWorkspaceDefaultsSnapshot.self, from: snapshotData)
    ...
    func test_loadState_backfillsLegacyWorkspaceBookmarkDataWhenAvailable() throws {
        ...
        let persistedSnapshot = try JSONDecoder().decode(CueLoopWorkspaceDefaultsSnapshot.self, from: persistedData)
        XCTAssertEqual(
    ...
    func test_unitTestDefaults_areIsolatedFromStandardDefaults() {
        let key = "com.mitchfultz.cueloop.unit-test-isolation"
        defer {
