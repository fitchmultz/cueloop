...
final class ErrorRecoveryCategoryTests: CueLoopCoreTestCase {
    ...
    func testRecoveryErrorCreation() {
        let error = RecoveryError(
            ...
            workspaceURL: CueLoopCoreTestSupport.workspaceURL(label: "recovery-error-creation")
        )
    ...
    func testClassifyCLIClientErrorNotFound() {
        let url = URL(fileURLWithPath: "/nonexistent/cueloop")
        let error = CueLoopCLIClientError.executableNotFound(url)
    ...
    func testClassifyCLIClientErrorNotExecutable() {
        let url = URL(fileURLWithPath: "/path/to/cueloop")
        let error = CueLoopCLIClientError.executableNotExecutable(url)
    ...
    func testClassifyLegacyCommandNotFoundAsCLIUnavailable() {
        let error = NSError(domain: "CueLoopCore.CLIProcess", code: 127, userInfo: [
            NSLocalizedDescriptionKey: "sh: cueloop: command not found"
        ])
    ...
    func testClassifyUnscopedMissingFileDoesNotBecomeCLIUnavailable() {
        let configPath = CueLoopCoreTestSupport.workspaceURL(label: "missing-config")
            .appendingPathComponent(".cueloop/config.jsonc", isDirectory: false)
            .path
        let error = NSError(domain: "CueLoopCore.CLIProcess", code: 1, userInfo: [
    ...
    func testClassifyUnsupportedConfigVersionAsConfigIncompatible() {
        let configPath = CueLoopCoreTestSupport.workspaceURL(label: "unsupported-config-version")
            .appendingPathComponent(".cueloop/config.jsonc", isDirectory: false)
            .path
        let error = RetryableError.processError(
            exitCode: 1,
            stderr: "Error: load project config \(configPath): Unsupported config version: 1. CueLoop requires version 2."
        )
    ...
    func testClassifyMachineErrorDocumentUsesStructuredCode() throws {
        let queuePath = CueLoopCoreTestSupport.workspaceURL(label: "machine-error-document")
            .appendingPathComponent(".cueloop/queue.jsonc", isDirectory: false)
            .path
    ...
    func testClassifyUserFacingMachineErrorSummaryPreservesStructuredMessage() {
        ...
        Retryable: yes
        """
        let error = NSError(domain: "CueLoopCore.CLIProcess", code: 23, userInfo: [
    ...
    func testClassifyDecodingErrorAsParseError() {
        ...
        do {
            _ = try JSONDecoder().decode(CueLoopTaskQueueDocument.self, from: payload)
    ...
    func testClassifyRetryableProcessErrorUsesStderr() {
        let queuePath = CueLoopCoreTestSupport.workspaceURL(label: "retryable-process-error")
            .appendingPathComponent(".cueloop/queue.jsonc", isDirectory: false)
            .path
        let error = RetryableError.processError(
            exitCode: 2,
    ...
    func testCanonicalClassifierAlignsGenericAndProcessFailures() {
        let queuePath = CueLoopCoreTestSupport.workspaceURL(label: "canonical-classifier")
            .appendingPathComponent(".cueloop/queue.jsonc", isDirectory: false)
            .path
    ...
    func testClassifyMissingQueueFileIsActionable() {
        let error = NSError(
            ...
            code: 2,
            userInfo: [
                NSLocalizedDescriptionKey: "Error: read queue file /Users/test/.cueloop/queue.jsonc: No such file or directory (os error 2)"
            ]
        )
    ...
    func testClassifyQueueLockHolderError() {
        let error = RetryableError.processError(
            exitCode: 1,
            stderr: """
            Queue lock already held at: /tmp/example/.cueloop/lock
    ...
    func testClassifyQueueCorruptedError() {
        let error = NSError(domain: "CueLoopError", code: 1, userInfo: [
