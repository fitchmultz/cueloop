...
final class WindowStateRoutingTests: WindowStateTestCase {
    func test_prepareForLaunch_prunesUITestingWorkspaceStateFromProductionDefaults() throws {
        ...
        let navigationKey = "com.mitchfultz.cueloop.navigationState.\(workspaceID.uuidString)"
        let snapshotKey = workspaceSnapshotKey(for: workspaceID)
        let cachedTasksKey = "com.mitchfultz.cueloop.workspace.\(workspaceID.uuidString).cachedTasks"
    ...
    func test_prepareForLaunch_unitTestingClearsStrayCurrentProcessDefaults() throws {
        ...
        let cachedTasksKey = "com.mitchfultz.cueloop.workspace.\(workspaceID.uuidString).cachedTasks"
        let navigationKey = "com.mitchfultz.cueloop.navigationState.\(workspaceID.uuidString)"
        let versionCacheKey = "com.mitchfultz.cueloop.versionCheckCache"
        let workspacePath = try makeWorkspaceDirectory(prefix: "unit-test-defaults-prune")
        defer { CueLoopCoreTestSupport.assertRemoved(workspacePath) }
    ...
    func test_scheduleWorkspaceReveal_resolvesWhenWindowRouteActionsRegister() async {
        ...
        let revealResolved = await CueLoopCoreTestSupport.waitUntil {
            await MainActor.run {
    ...
    func test_scheduleWorkspaceReveal_recordsTimeoutDiagnosticWhenAttemptsExhaust() async {
        ...
        let timeoutRecorded = await CueLoopCoreTestSupport.waitUntil {
            await MainActor.run {
                let manager = WorkspaceManager.shared
    ...
    func test_effectiveWorkspace_prefersFocusedAndLastActiveWorkspace() throws {
        ...
        defer {
            CueLoopCoreTestSupport.assertRemoved(firstDirectory)
            CueLoopCoreTestSupport.assertRemoved(secondDirectory)
        }
    ...
    func test_closeWorkspace_reassignsEffectiveWorkspaceWhenActiveWorkspaceCloses() throws {
        ...
        defer {
            CueLoopCoreTestSupport.assertRemoved(firstDirectory)
            CueLoopCoreTestSupport.assertRemoved(secondDirectory)
            CueLoopCoreTestSupport.assertRemoved(thirdDirectory)
        }
