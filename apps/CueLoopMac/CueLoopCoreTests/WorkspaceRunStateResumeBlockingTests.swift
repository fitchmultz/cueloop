...
final class WorkspaceRunStateResumeBlockingTests: WorkspacePerformanceTestCase {
    func test_loadRunnerConfiguration_appliesResumePreview() async throws {
        ...
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-resume-preview",
            scriptName: "mock-cueloop-resume-preview"
        )
    ...
    func test_machineResumeDecision_mapsCorruptSessionCacheToRunnerRecoveryBlocking() {
        let decision = MachineResumeDecision(
            ...
            detail: "Inspect .cueloop/cache/session.jsonc."
        )
    ...
    func test_loadTasks_appliesPreflightBlockingStateAndClearsNextTask() async throws {
        var workspace: Workspace!
        let blockedTask = CueLoopMockCLITestSupport.task(
            id: "RQ-9001",
            status: .todo,
        ...
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-blocking-preflight",
            scriptName: "mock-cueloop-blocking-preflight",
            seedQueueTasks: [blockedTask]
        )
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }

        let runnability = CueLoopJSONValue.object([
            "summary": .object([
                "blocking": .object([
    ...
    func test_runNextTask_appliesResumeDecisionEvent() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "resume-decision-event")
        )
    ...
    func test_runNextTask_deduplicatesRunnerRecoveryConsoleNarration() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "runner-recovery-event")
        )
        var decoder = WorkspaceRunnerController.MachineRunOutputDecoder()
        let items = decoder.append(
    ...
    func test_runNextTask_appliesBlockingStateEvent() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "blocking-state-event")
        )
        var decoder = WorkspaceRunnerController.MachineRunOutputDecoder()
        let items = decoder.append(
    ...
    func test_runSummary_appliesBlockingState() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "blocking-state-summary")
        )
        var decoder = WorkspaceRunnerController.MachineRunOutputDecoder()
        let items = decoder.append(
    ...
    func test_runSummary_stoppedWithoutBlocking_clearsExistingLiveBlockingState() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "summary-preserves-live-blocking")
        )
        var decoder = WorkspaceRunnerController.MachineRunOutputDecoder()
        let items = decoder.append(
    ...
    func test_runSummary_explicitBlocking_supersedesEarlierLiveBlockingState() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "summary-supersedes-live-blocking")
        )
        var decoder = WorkspaceRunnerController.MachineRunOutputDecoder()
        let items = decoder.append(
            "{\"version\":3,\"kind\":\"blocked_state_changed\",\"task_id\":null,\"phase\":null,\"message\":\"CueLoop is blocked by unfinished dependencies.\",\"payload\":{\"status\":\"blocked\",\"reason\":{\"kind\":\"dependency_blocked\",\"blocked_tasks\":2},\"task_id\":null,\"message\":\"CueLoop is blocked by unfinished dependencies.\",\"detail\":\"2 candidate task(s) are waiting on dependency completion.\",\"observed_at\":\"2026-12-30T00:00:00Z\"}}\n"
    ...
    func test_runSummary_completedOutcomeClearsExistingLiveBlockingState() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "summary-clears-live-blocking")
        )
        var decoder = WorkspaceRunnerController.MachineRunOutputDecoder()
        let items = decoder.append(
            "{\"version\":3,\"kind\":\"blocked_state_changed\",\"task_id\":null,\"phase\":null,\"message\":\"CueLoop is blocked by unfinished dependencies.\",\"payload\":{\"status\":\"blocked\",\"reason\":{\"kind\":\"dependency_blocked\",\"blocked_tasks\":2},\"task_id\":null,\"message\":\"CueLoop is blocked by unfinished dependencies.\",\"detail\":\"2 candidate task(s) are waiting on dependency completion.\"}}\n"
    ...
    func test_runSummary_failedOutcomeClearsExistingLiveBlockingState() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "summary-failed-clears-live-blocking")
        )
        var decoder = WorkspaceRunnerController.MachineRunOutputDecoder()
        let items = decoder.append(
            "{\"version\":3,\"kind\":\"blocked_state_changed\",\"task_id\":null,\"phase\":null,\"message\":\"CueLoop is blocked by unfinished dependencies.\",\"payload\":{\"status\":\"blocked\",\"reason\":{\"kind\":\"dependency_blocked\",\"blocked_tasks\":2},\"task_id\":null,\"message\":\"CueLoop is blocked by unfinished dependencies.\",\"detail\":\"2 candidate task(s) are waiting on dependency completion.\"}}\n"
    ...
    func test_runSummary_unknownOutcomePreservesExistingLiveBlockingState() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "summary-unknown-outcome")
        )
        var decoder = WorkspaceRunnerController.MachineRunOutputDecoder()
        let items = decoder.append(
            "{\"version\":3,\"kind\":\"blocked_state_changed\",\"task_id\":null,\"phase\":null,\"message\":\"CueLoop is blocked by unfinished dependencies.\",\"payload\":{\"status\":\"blocked\",\"reason\":{\"kind\":\"dependency_blocked\",\"blocked_tasks\":2},\"task_id\":null,\"message\":\"CueLoop is blocked by unfinished dependencies.\",\"detail\":\"2 candidate task(s) are waiting on dependency completion.\"}}\n"
    ...
    func test_runControlPreviewTask_prefersSelectedTodoTask() {
        let workspace = Workspace(workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "run-control-preview"))
        workspace.tasks = [
            CueLoopTask(id: "RQ-1001", status: .todo, title: "First", priority: .medium),
            CueLoopTask(id: "RQ-1002", status: .todo, title: "Second", priority: .high)
        ]
