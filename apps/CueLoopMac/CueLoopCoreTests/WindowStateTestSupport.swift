...
@MainActor
class WindowStateTestCase: CueLoopCoreTestCase {
    let testRestorationKey = "com.mitchfultz.cueloop.windowRestorationState"
    let testNavigationKey = "com.mitchfultz.cueloop.navigationState"
    let workspaceSnapshotPrefix = "com.mitchfultz.cueloop.workspace."

    var manager: WorkspaceManager!
    var defaults: UserDefaults { CueLoopAppDefaults.userDefaults }
    ...
    func makeSeededWorkspaceDirectory(prefix: String) throws -> URL {
        ...
        try #"{"version":1,"tasks":[]}"#.write(
            to: cueloopDirectory.appendingPathComponent("queue.jsonc", isDirectory: false),
            atomically: true,
            encoding: .utf8
        )
        return directory
