...
final class WorkspaceRunnerConfigurationLoadingTests: WorkspacePerformanceTestCase {
    func test_loadRunnerConfiguration_setsCurrentRunnerConfig() async throws {
        ...
        let fixture = try CueLoopMockCLITestSupport.makeFixture(prefix: "cueloop-workspace-config")
        ...
        let script = """
            ...
            exit 64
            """
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(in: fixture.rootURL, body: script)
    ...
    func test_loadRunnerConfiguration_decodesSafetySummary() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(prefix: "cueloop-workspace-config-safety")
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
        ...
        let script = """
            ...
            exit 64
            """
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(in: fixture.rootURL, name: "mock-cueloop-safety", body: script)
        let client = try CueLoopCLIClient(executableURL: scriptURL)
        workspace = CueLoopMockCLITestSupport.makeWorkspaceWithoutInitialRefresh(
            workingDirectoryURL: fixture.workspaceURL,
            client: client
    ...
    func test_loadRunnerConfiguration_preservesUnknownConfiguredRunnerAndEffort() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(prefix: "cueloop-workspace-config-unknown")
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: fixture.rootURL,
            name: "mock-cueloop-config-unknown",
            body: script
        )
        let client = try CueLoopCLIClient(executableURL: scriptURL)
        workspace = CueLoopMockCLITestSupport.makeWorkspaceWithoutInitialRefresh(
    ...
    func test_loadRunnerConfiguration_onFailure_clearsCurrentRunnerConfig() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(prefix: "cueloop-workspace-config-failure")
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
        ...
        let successScript = """
            ...
            exit 64
            """
        let successScriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: fixture.rootURL,
            name: "mock-cueloop-success",
            body: successScript
        )
        let successClient = try CueLoopCLIClient(executableURL: successScriptURL)
        workspace = CueLoopMockCLITestSupport.makeWorkspaceWithoutInitialRefresh(
            workingDirectoryURL: fixture.workspaceURL,
        ...
        let failScript = """
            ...
            exit 1
            """
        let failScriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: fixture.rootURL,
            name: "mock-cueloop-fail",
            body: failScript
        )
        let failClient = try CueLoopCLIClient(executableURL: failScriptURL)
    ...
    func test_shutdown_prevents_runnerConfiguration_reload_activity() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(prefix: "cueloop-workspace-config-shutdown")
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
