...
final class WindowStateRestorationTests: WindowStateTestCase {
    ...
    func test_restoreWindows_withNoSavedState_usesExistingWorkspace() throws {
        ...
        defer { CueLoopCoreTestSupport.assertRemoved(temp) }
    ...
    func test_restoreWindows_withValidSavedState_restoresCorrectly() throws {
        ...
        defer {
            CueLoopCoreTestSupport.assertRemoved(directory1)
            CueLoopCoreTestSupport.assertRemoved(directory2)
        }
    ...
    func test_restoreWindows_doesNotBootstrapRestoredWorkspacesBeforeTheyAppear() async throws {
        ...
        defer { CueLoopCoreTestSupport.assertRemoved(fixture.rootURL) }
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: fixture.rootURL,
            name: "mock-cueloop-restore-no-eager-bootstrap",
            body: script
        )
    ...
    func test_workspaceProjectDisplayName_prefersWorkingDirectoryLeafName() throws {
        ...
        workspace.identityState.name = "CueLoopMac"
    ...
    func test_setWorkingDirectory_consumesURLRoutingPlaceholderDisposition() throws {
        ...
        defer {
            CueLoopCoreTestSupport.assertRemoved(initialDirectory)
            CueLoopCoreTestSupport.assertRemoved(targetDirectory)
        }
    ...
    func test_workspaceIsRestorable_acceptsConfigOnlyWorkspaceForCustomQueuePath() throws {
        ...
        let cueloopDirectory = directory.appendingPathComponent(".cueloop", isDirectory: true)
        try FileManager.default.createDirectory(at: cueloopDirectory, withIntermediateDirectories: true)
        try """
        {
          "version": 2,
          "queue": {
        ...
        """.write(
            to: cueloopDirectory.appendingPathComponent("config.jsonc", isDirectory: false),
            atomically: true,
            encoding: .utf8
        )
    ...
    func test_workspaceIsRestorable_acceptsCueLoopDirectoryWithoutLocalQueueConfig() throws {
        let directory = try makeWorkspaceDirectory(prefix: "workspace-restorable-global-config")
        defer { CueLoopCoreTestSupport.assertRemoved(directory) }

        try FileManager.default.createDirectory(
            at: directory.appendingPathComponent(".cueloop", isDirectory: true),
    ...
    func test_scheduleInitialRepositoryBootstrapIfNeeded_bootstrapsRestoredWorkspaceOnDemand() async throws {
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: fixture.rootURL,
            name: "mock-cueloop-on-demand-bootstrap",
            body: script
        )
