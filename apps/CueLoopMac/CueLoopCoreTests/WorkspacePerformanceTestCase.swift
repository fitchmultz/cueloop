...
@MainActor
class WorkspacePerformanceTestCase: CueLoopCoreTestCase {
    var workspace: Workspace!

    override func setUp() async throws {
        try await super.setUp()
        workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(
    ...
    func generateTasks(count: Int) -> [CueLoopTask] {
    ...
    func generateTasks(count: Int, mutateFrom base: [CueLoopTask]) -> [CueLoopTask] {
    ...
    func generateTasksWithDependencies(count: Int) -> [CueLoopTask] {
        (1...count).map { index in
