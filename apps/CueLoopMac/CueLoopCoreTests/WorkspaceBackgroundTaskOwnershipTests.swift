...
final class WorkspaceBackgroundTaskOwnershipTests: CueLoopCoreTestCase {
    @MainActor
    ...
    func test_closeWorkspace_cancelsInFlightCLISpecBootstrap() async throws {
        ...
        let rootDir = try WorkspacePerformanceTestSupport.makeTempDir(prefix: "cueloop-background-bootstrap-close")
        defer { CueLoopCoreTestSupport.assertRemoved(rootDir) }
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: rootDir,
            name: "mock-cueloop-cli-spec-bootstrap",
            body: script
        )
    ...
    func test_createWorkspace_doesNotScheduleAutomaticCLISpecBootstrap() throws {
        ...
        let rootDir = try WorkspacePerformanceTestSupport.makeTempDir(prefix: "cueloop-background-bootstrap-auto")
        defer { CueLoopCoreTestSupport.assertRemoved(rootDir) }
        let workspaceURL = rootDir.appendingPathComponent("workspace", isDirectory: true)
        try FileManager.default.createDirectory(at: workspaceURL, withIntermediateDirectories: true)
    ...
    func test_scheduleHealthCheck_shutdownCancelsInFlightProbe() async throws {
        ...
        let rootDir = try WorkspacePerformanceTestSupport.makeTempDir(prefix: "cueloop-background-health-shutdown")
        defer { CueLoopCoreTestSupport.shutdownAndRemove(rootDir, workspace) }
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: rootDir,
            name: "mock-cueloop-health-shutdown",
            body: script
        )
        let client = try CueLoopCLIClient(executableURL: scriptURL)
        ...
        let shutdownSettled = await CueLoopCoreTestSupport.waitUntil(timeout: .seconds(3)) {
            await MainActor.run {
    ...
    func test_scheduleHealthCheck_retargetDiscardsStaleProbeResult() async throws {
        let rootDir = try WorkspacePerformanceTestSupport.makeTempDir(prefix: "cueloop-background-health-retarget")
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: rootDir,
            name: "mock-cueloop-health-retarget",
            body: script
        )
        let client = try CueLoopCLIClient(executableURL: scriptURL)
        ...
        let firstStarted = await CueLoopCoreTestSupport.waitUntil(timeout: .seconds(2)) {
        ...
        let retargeted = await CueLoopCoreTestSupport.waitUntil(timeout: .seconds(3)) {
            await MainActor.run {
    ...
    func test_shutdown_cancelsPendingRunLaunchBeforeProcessStarts() async throws {
        var workspace: Workspace!
        let rootDir = try WorkspacePerformanceTestSupport.makeTempDir(prefix: "cueloop-background-run-shutdown")
        defer { CueLoopCoreTestSupport.shutdownAndRemove(rootDir, workspace) }
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: rootDir,
            name: "mock-cueloop-run-shutdown",
            body: script
        )
        let client = try CueLoopCLIClient(executableURL: scriptURL)
        workspace = Workspace(workingDirectoryURL: rootDir)
    ...
    func test_setWorkingDirectory_cancelsPendingRunNextTaskResolution() async throws {
        var workspace: Workspace!
        let rootDir = try WorkspacePerformanceTestSupport.makeTempDir(prefix: "cueloop-background-run-retarget")
        defer { CueLoopCoreTestSupport.shutdownAndRemove(rootDir, workspace) }
        let workspaceAURL = rootDir.appendingPathComponent("workspace-a", isDirectory: true)
        let workspaceBURL = rootDir.appendingPathComponent("workspace-b", isDirectory: true)
        try FileManager.default.createDirectory(at: workspaceAURL, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: workspaceBURL, withIntermediateDirectories: true)
        try CueLoopMockCLITestSupport.writeQueueFile(in: workspaceAURL, tasks: [])
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: rootDir,
            name: "mock-cueloop-run-retarget",
            body: script
        )
        let client = try CueLoopCLIClient(executableURL: scriptURL)
        ...
        let runCommandStarted = await CueLoopCoreTestSupport.waitUntil(timeout: .seconds(2)) {
        ...
        let retargetSettled = await CueLoopCoreTestSupport.waitUntil(timeout: .seconds(3)) {
            await MainActor.run {
                workspace.identityState.workingDirectoryURL == workspaceBURL
        ...
        let pidRecorded = await CueLoopCoreTestSupport.waitForFile(pidFileURL, timeout: .seconds(2))
        XCTAssertTrue(pidRecorded)

        let pidText = try String(contentsOf: pidFileURL, encoding: .utf8)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let pid = pid_t(try XCTUnwrap(Int32(pidText)))
        let cancelled = await CueLoopCoreTestSupport.waitForProcessExit(pid, timeout: .seconds(5))
