...
final class WorkspaceRunNextInvocationTests: WorkspacePerformanceTestCase {
    func test_runNextTask_entersPreparationWithoutQueueReadOrImplicitTaskID() async throws {
        ...
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-run-preparing",
            scriptName: "mock-cueloop-run-preparing",
            seedQueueTasks: []
        )
    ...
    func test_runNextTask_usesCanonicalCLISelection_andStreamsSelectedTask() async throws {
        var workspace: Workspace!
        let stalePreviewTask = CueLoopMockCLITestSupport.task(
            id: "RQ-OLD",
            status: .todo,
        ...
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-run-stream",
            scriptName: "mock-cueloop-run-stream",
    ...
    func test_runNextTask_withExplicitIDAndForce_usesExpectedArguments() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-run-explicit",
            scriptName: "mock-cueloop-run-explicit"
        )
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
        let commandLogURL = fixture.rootURL.appendingPathComponent("command-log.txt", isDirectory: false)
