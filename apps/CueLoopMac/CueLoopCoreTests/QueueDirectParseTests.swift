/**
 ...
 Responsibilities:
 - Validate direct queue file parsing for file-watcher-triggered refreshes.
 - Ensure CueLoopTaskQueueDocument correctly decodes valid queue files.
...
final class QueueDirectParseTests: CueLoopCoreTestCase {
    ...
    func test_decode_validQueueDocument_succeeds() throws {
        ...
        let document = try decoder.decode(CueLoopTaskQueueDocument.self, from: Data(json.utf8))
    ...
    func test_decode_legacyArrayFormat_throwsError() {
        ...
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        XCTAssertThrowsError(try decoder.decode(CueLoopTaskQueueDocument.self, from: Data(json.utf8)))
    }
