...
final class WorkspaceTaskMutationAgentTests: WorkspacePerformanceTestCase {
  func test_updateTask_agentOverride_emitsAgentEditCommand() async throws {
    let fixture = try Self.makeMutationFixture(
      prefix: "cueloop-workspace-agent-edit",
      scriptName: "mock-cueloop-task-mutate-agent",
      mutationReportJSON:
    ...
    updated.agent = CueLoopTaskAgent(
      runner: "codex",
      model: "gpt-5.3-codex",
      modelEffort: "high",
      phases: 2,
  ...
  func test_updateTask_multipleFieldChanges_emitSharedEncodedEdits() async throws {
    let fixture = try Self.makeMutationFixture(
      prefix: "cueloop-workspace-multi-edit",
      scriptName: "mock-cueloop-task-mutate-multi-edit",
      mutationReportJSON:
    ...
    updated.scope = ["apps/CueLoopMac", "CueLoopCore"]
    ...
    let decodedAgent = try JSONDecoder().decode(
      CueLoopTaskAgent.self, from: Data(agentEdit.value.utf8))
    XCTAssertEqual(decodedAgent, CueLoopTaskAgent.normalizedOverride(updated.agent))
  }

  func test_updateTask_clearingAgentOverride_emitsEmptyAgentValue() async throws {
    let fixture = try Self.makeMutationFixture(
      prefix: "cueloop-workspace-agent-clear",
      scriptName: "mock-cueloop-task-mutate-agent-clear",
      mutationReportJSON:
  ...
  func test_updateTask_semanticallyEmptyAgentOverride_doesNotEmitAgentEdit() async throws {
    let fixture = try Self.makeMutationFixture(
      prefix: "cueloop-workspace-agent-noop",
      scriptName: "mock-cueloop-task-mutate-agent-noop",
      mutationReportJSON:
  ...
  private static func makeMutationFixture(
    ...
    _ = try CueLoopMockCLITestSupport.makeExecutableScript(
