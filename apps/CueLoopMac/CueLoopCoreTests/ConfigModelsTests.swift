/**
 ...
 Purpose:
 - Regression-test decoding of CueLoopCore config models against CLI-shaped JSON payloads.
...
final class ConfigModelsTests: CueLoopCoreTestCase {
    func test_decode_machineConfigResolve_includesWebhookUrlPolicyFields() throws {
        let json = #"""
        {
          ...
          "paths": {
            "repo_root": "/tmp/ws",
            "queue_path": "/tmp/ws/.cueloop/queue.jsonc",
            "done_path": "/tmp/ws/.cueloop/done.jsonc",
            "project_config_path": "/tmp/ws/.cueloop/config.jsonc",
          ...
          "config": {
            "agent": {
              ...
              "webhook": {
                "enabled": true,
                "url": "https://hooks.example.com/cueloop",
            ...
            "diagnostics": [
              {
                ...
                "detail": "failed to parse .cueloop/plugins/broken.runner/plugin.json",
        ...
        XCTAssertEqual(webhook.url, "https://hooks.example.com/cueloop")
    ...
    func test_decode_cueloopConfig_notification_includesWatchNewTasksField() throws {
        let json = #"""
        {
          "agent": {
            "notification": {
        ...
        let config = try JSONDecoder().decode(CueLoopConfig.self, from: Data(json.utf8))
