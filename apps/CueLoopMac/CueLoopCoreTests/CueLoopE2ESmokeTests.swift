/**
 ...
 - A deterministic `cueloop` binary must be available via either:
   - `CUELOOP_BIN_PATH`, or
   - the bundled app binary at `CueLoopMac.app/Contents/MacOS/cueloop`.
 - A Rust toolchain is available if `CUELOOP_E2E_ALLOW_CARGO_BUILD=1` enables fallback cargo builds.
...
final class CueLoopE2ESmokeTests: CueLoopCoreTestCase {
    private static let allowCargoBuildEnvKey = "CUELOOP_E2E_ALLOW_CARGO_BUILD"
    ...
    func test_e2e_smoke_version_init_and_machineQueueRead_json() async throws {
        let cueloopURL = try Self.resolveCueLoopBinaryURL()
        let client = try CueLoopCLIClient(executableURL: cueloopURL)
        ...
        let tempDir = try Self.makeTempDir(prefix: "cueloop-e2e-")
    ...
    func test_resolveCueLoopBinaryURL_envOverride_success() throws {
        let tempDir = try Self.makeTempDir(prefix: "cueloop-resolver-env-")
        defer { CueLoopCoreTestSupport.assertRemoved(tempDir) }

        let binaryURL = tempDir.appendingPathComponent("cueloop", isDirectory: false)
        try Self.writeExecutableScript(at: binaryURL)

        let resolved = try Self.resolveCueLoopBinaryURL(
    ...
    func test_resolveCueLoopBinaryURL_missingEnv_fallbackDisabled_failsFast() throws {
        let tempDir = try Self.makeTempDir(prefix: "cueloop-resolver-no-fallback-")
        defer { CueLoopCoreTestSupport.assertRemoved(tempDir) }

        XCTAssertThrowsError(
            try Self.resolveCueLoopBinaryURL(
                environment: [:],
                repoRoot: tempDir,
                bundledBinaryURL: nil,
    ...
    func test_resolveCueLoopBinaryURL_fallbackEnabled_failsFastWithoutCargoBuild() throws {
        let tempDir = try Self.makeTempDir(prefix: "cueloop-resolver-fallback-")
        defer { CueLoopCoreTestSupport.assertRemoved(tempDir) }
    ...
    private struct Collected {
        let status: CueLoopCLIExitStatus
        let stdout: String
        let stderr: String
    }
    ...
    private static func runAndCollect(
        ...
        do {
            ...
            return Collected(
                status: CueLoopCLIExitStatus(code: -1, reason: .exit),
                stdout: "",
    ...
    private static func resolveCueLoopBinaryURL() throws -> URL {
        try resolveCueLoopBinaryURL(environment: ProcessInfo.processInfo.environment)
    }

    private static func resolveCueLoopBinaryURL(
        environment: [String: String],
        repoRoot: URL? = nil,
        bundledBinaryURL: URL? = bundledCueLoopBinaryURL(),
        ...
        guard environment[allowCargoBuildEnvKey] == "1" else {
            throw resolverError(
                "Missing \(binaryPathEnvKey). Set \(binaryPathEnvKey) to an executable `cueloop` binary for deterministic tests, " +
                    "or ensure the bundled CueLoop binary is present at \(bundledCueLoopBinaryPathDescription()). " +
    ...
    private static func resolverError(_ message: String) -> NSError {
        NSError(domain: "CueLoopE2E", code: 2, userInfo: [NSLocalizedDescriptionKey: message])
    }

    private static func bundledCueLoopBinaryURL() -> URL? {
        let bundleURL = Bundle(for: CueLoopE2ESmokeTests.self).bundleURL
    ...
    private static func bundledCueLoopBinaryPathDescription() -> String {
        bundledCueLoopBinaryURL()?.path ?? "<derived-data>/Build/Products/Debug/CueLoopMac.app/Contents/MacOS/cueloop"
    }
    ...
    private static func findRepoRoot(startingAt url: URL) throws -> URL {
        ...
        while true {
            ...
            if parent.path == current.path {
                throw NSError(domain: "CueLoopE2E", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to locate repo root"])
            }
            current = parent
        }
    }
