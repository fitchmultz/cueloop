/**
 ...
 - Tests exercise the low-level watcher directly against isolated `.cueloop` fixtures.
 */
...
@MainActor
final class QueueFileWatcherTests: CueLoopCoreTestCase {
    func test_queueFileWatcher_rapidStartStopWithMutationsDoesNotCrash() async throws {
        for index in 0..<20 {
            let workspaceURL = try WorkspaceTaskCreationTestSupport.makeTempDir(prefix: "cueloop-workspace-watcher-")
            defer { CueLoopCoreTestSupport.assertRemoved(workspaceURL) }
            let cueloopURL = try WorkspaceTaskCreationTestSupport.prepareWatcherFixture(at: workspaceURL)
            ...
            await watcher.start()
            let queueURL = cueloopURL.appendingPathComponent("queue.jsonc", isDirectory: false)
            try """
            [
    ...
    func test_queueFileWatcher_stopBeforeDebounceSuppressesNotification() async throws {
        let workspaceURL = try WorkspaceTaskCreationTestSupport.makeTempDir(prefix: "cueloop-workspace-watcher-stop-")
        defer { CueLoopCoreTestSupport.assertRemoved(workspaceURL) }
        ...
        """.write(
            to: cueloopURL.appendingPathComponent("queue.jsonc", isDirectory: false),
            atomically: true,
            encoding: .utf8
        )
        await watcher.stop()
    ...
    func test_queueFileWatcher_replaceItemAtQueueFileEmitsNotification() async throws {
        let workspaceURL = try WorkspaceTaskCreationTestSupport.makeTempDir(prefix: "cueloop-workspace-watcher-replace-")
        defer { CueLoopCoreTestSupport.assertRemoved(workspaceURL) }
        ...
        try WorkspaceTaskCreationTestSupport.writeQueueDocument(
            to: queueURL,
            tasks: [
                CueLoopTask(id: "RQ-INITIAL", status: .todo, title: "Initial", priority: .medium)
            ]
        )
        ...
        try WorkspaceTaskCreationTestSupport.writeQueueDocument(
            to: replacementURL,
            tasks: [
                CueLoopTask(id: "RQ-REPLACED", status: .todo, title: "Replacement", priority: .high)
            ]
        )
    ...
    func test_queueFileWatcher_customTargetsEmitNotificationForConfiguredQueuePath() async throws {
        let workspaceURL = try WorkspaceTaskCreationTestSupport.makeTempDir(prefix: "cueloop-workspace-watcher-custom-")
        defer { CueLoopCoreTestSupport.assertRemoved(workspaceURL) }
        ...
        let configURL = workspaceURL.appendingPathComponent(".cueloop/config.jsonc", isDirectory: false)
        ...
        try WorkspaceTaskCreationTestSupport.writeQueueDocument(
            to: queueURL,
            tasks: [
                CueLoopTask(id: "RQ-CUSTOM", status: .todo, title: "Configured target", priority: .medium)
            ]
        )
    ...
    func test_queueFileWatcher_customQueueFileNameStillMarksQueueSnapshotChanges() async throws {
        let workspaceURL = try WorkspaceTaskCreationTestSupport.makeTempDir(prefix: "cueloop-workspace-watcher-custom-name-")
        defer { CueLoopCoreTestSupport.assertRemoved(workspaceURL) }
        ...
        try WorkspaceTaskCreationTestSupport.writeQueueDocument(
            to: queueURL,
            tasks: [CueLoopTask(id: "RQ-CUSTOM-NAME", status: .todo, title: "Custom queue file", priority: .medium)]
        )
    ...
    func test_queueFileWatcher_surfacesFailureAfterRetryExhaustion() async throws {
        let workspaceURL = try WorkspaceTaskCreationTestSupport.makeTempDir(prefix: "cueloop-workspace-watcher-fail-")
        defer { CueLoopCoreTestSupport.assertRemoved(workspaceURL) }
