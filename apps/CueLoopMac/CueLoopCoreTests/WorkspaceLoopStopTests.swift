...
final class WorkspaceLoopStopTests: WorkspacePerformanceTestCase {
    func test_stopLoop_requestsQueueStopSignalForActiveMachineLoop() async throws {
        ...
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-loop-stop",
            scriptName: "mock-cueloop-loop-stop",
            seedQueueTasks: []
        )
        ...
        let script = """
            ...
            case "$*" in
              ...
              *"--no-color machine run stop"*)
              ...
              printf '%s\n' '{"version":1,"dry_run":false,"action":"created","paths":{"repo_root":"\(fixture.workspaceURL.path)","queue_path":"\(fixture.workspaceURL.path)/.cueloop/queue.jsonc","done_path":"\(fixture.workspaceURL.path)/.cueloop/done.jsonc","project_config_path":"\(fixture.workspaceURL.path)/.cueloop/config.jsonc","global_config_path":null},"marker":{"path":"\(fixture.workspaceURL.path)/.cueloop/cache/stop_requested","existed_before":false,"exists_after":true},"blocking":null,"continuation":{"headline":"Stop request recorded.","detail":"The stop marker is recorded.","next_steps":[]}}'
              exit 0
              ;;
            esac
    ...
    func test_stopLoop_keepsStopRequestedWhenMachineRunStopReportsAlreadyPresent() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-loop-stop-already-present",
            scriptName: "mock-cueloop-loop-stop-already-present",
            seedQueueTasks: []
        )
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
        let commandLogURL = fixture.rootURL.appendingPathComponent("command-log.txt", isDirectory: false)
        ...
        let script = """
            ...
            case "$*" in
              ...
              *"--no-color machine run stop"*)
              echo "stop command received" > "\(stopSignalURL.path)"
              printf '%s\n' '{"version":1,"dry_run":false,"action":"already_present","paths":{"repo_root":"\(fixture.workspaceURL.path)","queue_path":"\(fixture.workspaceURL.path)/.cueloop/queue.jsonc","done_path":"\(fixture.workspaceURL.path)/.cueloop/done.jsonc","project_config_path":"\(fixture.workspaceURL.path)/.cueloop/config.jsonc","global_config_path":null},"marker":{"path":"\(fixture.workspaceURL.path)/.cueloop/cache/stop_requested","existed_before":true,"exists_after":true},"blocking":null,"continuation":{"headline":"Stop request is already recorded.","detail":"The stop marker already exists.","next_steps":[]}}'
              exit 0
              ;;
            esac
    ...
    func test_stopLoop_preservesExistingBlockingWhenStopDocumentHasNoBlocking() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-loop-stop-preserve-blocking",
            scriptName: "mock-cueloop-loop-stop-preserve-blocking",
            seedQueueTasks: []
        )
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
        let stopSignalURL = fixture.rootURL.appendingPathComponent("stop-signal.txt", isDirectory: false)
    ...
    func test_runControlOperatorState_exposesStopAfterCurrentActionDuringLoopBlocking() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "run-control-stop-after-current-action")
        )
    ...
    func test_stopLoop_structuredFailureClearsStopRequestedAndShowsRecoveryError() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-loop-stop-failure",
            scriptName: "mock-cueloop-loop-stop-failure",
            seedQueueTasks: []
        )
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
