...
final class WorkspaceParallelRunControlTests: WorkspacePerformanceTestCase {
    func test_loadParallelStatus_decodesSharedContinuationDocument() async throws {
        ...
        let fixture = try CueLoopMockCLITestSupport.makeFixture(prefix: "cueloop-workspace-parallel-status")
        ...
        try """
            {"version":3,"lifecycle_counts":{"running":1,"integrating":0,"completed":0,"failed":0,"blocked":0,"total":1},"blocking":null,"continuation":{"headline":"Parallel execution is in progress.","detail":"Parallel workers are active on target branch main.","blocking":null,"next_steps":[{"title":"Inspect worker snapshot","command":"cueloop machine run parallel-status","detail":"Review lifecycle counts and retained worker details."}]},"status":{"schema_version":3,"target_branch":"main","workers":[{"task_id":"RQ-7001","workspace_path":"\(fixture.workspaceURL.appendingPathComponent(".cueloop/workspaces/RQ-7001", isDirectory: true).path)","lifecycle":"running","started_at":"2026-03-22T00:00:00Z","push_attempts":1}]}}
    ...
    func test_loadParallelStatus_rejectsUnsupportedVersion_andClearsRetainedState() async throws {
        try await assertParallelStatusLoadFailure(
            prefix: "cueloop-workspace-parallel-version-failure",
            payload: """
    ...
    func test_loadParallelStatus_rejectsMalformedJSON_andClearsRetainedState() async throws {
        try await assertParallelStatusLoadFailure(
            prefix: "cueloop-workspace-parallel-json-failure",
            payload: """
    ...
    func test_loadParallelStatus_rejectsMissingRequiredFields_andClearsRetainedState() async throws {
        try await assertParallelStatusLoadFailure(
            prefix: "cueloop-workspace-parallel-missing-field-failure",
            payload: """
    ...
    func test_runState_runControlOperatorState_appliesPrecedence_andDeduplicatesResumeRecovery() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "run-control-display-blocking")
        )
    ...
    func test_runState_shouldShowRunControlParallelStatus_tracksLoadingErrorConfiguredAndRetainedState() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "run-control-parallel-visibility")
        )
    ...
    func test_runState_runControlOperatorState_prefersParallelBlockingOverQueueSnapshot() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "run-control-parallel-precedence")
        )
    ...
    func test_runState_runControlOperatorState_classifiesQueueRecoveryActions() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "run-control-queue-recovery-actions")
        )
    ...
    func test_loadRunnerConfiguration_failure_preservesExistingQueueOperatorState() async throws {
        ...
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-run-control-preserve-state",
            scriptName: "mock-cueloop-run-control-preserve-state"
        )
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
        ...
        let script = """
            #!/bin/sh
            echo "Queue lock already held at: \(fixture.workspaceURL.path)/.cueloop/lock" 1>&2
            exit 1
            """
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: fixture.rootURL,
    ...
    func test_refreshRunControlStatusData_reloadsConfigAndParallelStatusWithoutQueueRead() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-run-control-status-refresh",
        ...
        let seededTask = CueLoopMockCLITestSupport.task(
            id: "RQ-2222",
            status: .todo,
            title: "Leave task state alone",
            priority: .medium,
    ...
    func test_refreshRunControlQueueStatusData_readsQueueBeforeRefreshingRunControlState() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-run-control-queue-refresh",
            logFileName: "commands.log",
            seedQueueTasks: [
                CueLoopMockCLITestSupport.task(
                    id: "RQ-3333",
        ...
        let configResolveURL = try CueLoopMockCLITestSupport.writeJSONDocument(
            CueLoopMockCLITestSupport.configResolveDocument(workspaceURL: fixture.workspaceURL),
            in: fixture.rootURL,
            name: "config-resolve.json"
        )
    ...
    func test_refreshRepositoryState_clearsInactiveParallelStatusWhenParallelNotConfigured() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-parallel-refresh-clear",
            logFileName: "commands.log"
        )
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
    ...
    func test_refreshRepositoryState_reloadsMeaningfulParallelStatusWhenParallelNotConfigured() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-parallel-refresh-reload",
            logFileName: "commands.log"
        )
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
        ...
        let parallelStatusURL = fixture.rootURL.appendingPathComponent("parallel-status.json", isDirectory: false)
        try """
            {"version":3,"lifecycle_counts":{"running":1,"integrating":0,"completed":0,"failed":0,"blocked":0,"total":1},"blocking":null,"continuation":{"headline":"Parallel execution is in progress.","detail":"Retained worker state still exists for this repository.","blocking":null,"next_steps":[{"title":"Inspect worker snapshot","command":"cueloop machine run parallel-status","detail":"Review retained worker lifecycles."}]},"status":{"schema_version":3,"target_branch":"main","workers":[{"task_id":"RQ-8111","workspace_path":"\(fixture.workspaceURL.appendingPathComponent(".cueloop/workspaces/RQ-8111", isDirectory: true).path)","lifecycle":"running","started_at":"2026-03-22T00:00:00Z","push_attempts":1}]}}
            """.write(to: parallelStatusURL, atomically: true, encoding: .utf8)
    ...
    func test_refreshRepositoryState_reloadsParallelStatusWhenRunControlOverrideRequestsParallelLoop() async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(
            prefix: "cueloop-workspace-parallel-refresh-override",
            logFileName: "commands.log"
        )
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
    ...
    func test_beginRepositoryRetarget_clearsParallelStatusState() {
        let workspace = Workspace(
            workingDirectoryURL: CueLoopCoreTestSupport.workspaceURL(label: "parallel-status-retarget")
        )
        workspace.runState.parallelStatus = Workspace.ParallelStatus(
        ...
        _ = workspace.beginRepositoryRetarget(
            to: CueLoopCoreTestSupport.workspaceURL(label: "parallel-status-retarget-next")
        )
    ...
    private func assertParallelStatusLoadFailure(prefix: String, payload: String) async throws {
        var workspace: Workspace!
        let fixture = try CueLoopMockCLITestSupport.makeFixture(prefix: prefix)
        defer { CueLoopCoreTestSupport.shutdownAndRemove(fixture.rootURL, workspace) }
