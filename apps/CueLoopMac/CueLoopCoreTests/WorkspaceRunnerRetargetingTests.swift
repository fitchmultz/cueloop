...
final class WorkspaceRunnerRetargetingTests: WorkspacePerformanceTestCase {
    func test_workspaceBootstrap_loadsTasksAndRunnerConfigurationWithoutGraphAnalyticsOrCLISpec() async throws {
        ...
        let rootDir = try WorkspacePerformanceTestSupport.makeTempDir(prefix: "cueloop-workspace-bootstrap-minimal")
        ...
        let task = CueLoopMockCLITestSupport.task(
            id: "RQ-BOOT",
            status: .todo,
        ...
        try CueLoopMockCLITestSupport.writeQueueFile(in: workspaceDir, tasks: [task])
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: rootDir,
            name: "mock-cueloop-bootstrap-minimal",
            body: script
        )
    ...
    func test_setWorkingDirectory_refreshesRunnerConfiguration() async throws {
        var workspace: Workspace!
        let rootDir = try WorkspacePerformanceTestSupport.makeTempDir(prefix: "cueloop-workspace-config-switch")
        defer { CueLoopCoreTestSupport.shutdownAndRemove(rootDir, workspace) }
    ...
    func test_setWorkingDirectory_clearsRepositoryDerivedStateImmediately_andReloadsNewRepository() async throws {
        var workspace: Workspace!
        let rootDir = try WorkspacePerformanceTestSupport.makeTempDir(prefix: "cueloop-workspace-retarget")
        defer { CueLoopCoreTestSupport.shutdownAndRemove(rootDir, workspace) }
        let workspaceADir = rootDir.appendingPathComponent("workspace-a", isDirectory: true)
        let workspaceBDir = rootDir.appendingPathComponent("workspace-b", isDirectory: true)
        try FileManager.default.createDirectory(at: workspaceADir, withIntermediateDirectories: true)
        ...
        let workspaceATask = CueLoopMockCLITestSupport.task(
            id: "RQ-A",
            status: .todo,
        ...
        let workspaceBTask = CueLoopMockCLITestSupport.task(
            id: "RQ-B",
            status: .todo,
        ...
        try CueLoopMockCLITestSupport.writeQueueFile(in: workspaceADir, tasks: [workspaceATask])
        try CueLoopMockCLITestSupport.writeQueueFile(in: workspaceBDir, tasks: [workspaceBTask])
        ...
        let graphAURL = try WorkspaceRunnerConfigurationTestSupport.writeGraphDocument(
            in: rootDir,
            name: "graph-a.json",
            tasks: [CueLoopMockCLITestSupport.graphNode(id: "RQ-A", title: "Graph A")]
        )
        let graphBURL = try WorkspaceRunnerConfigurationTestSupport.writeGraphDocument(
            in: rootDir,
            name: "graph-b.json",
            tasks: [CueLoopMockCLITestSupport.graphNode(id: "RQ-B", title: "Graph B")]
        )
    ...
    func test_repositoryGeneration_discardsLateResultsFromPreviousWorkspace() async throws {
        var workspace: Workspace!
        let rootDir = try WorkspacePerformanceTestSupport.makeTempDir(prefix: "cueloop-workspace-retarget-stale")
        defer { CueLoopCoreTestSupport.shutdownAndRemove(rootDir, workspace) }
        let workspaceADir = rootDir.appendingPathComponent("workspace-a", isDirectory: true)
        let workspaceBDir = rootDir.appendingPathComponent("workspace-b", isDirectory: true)
        try FileManager.default.createDirectory(at: workspaceADir, withIntermediateDirectories: true)
        ...
        let staleTask = CueLoopMockCLITestSupport.task(
            id: "RQ-A",
            status: .todo,
            title: "Stale A Task",
            priority: .high,
        ...
        let freshTask = CueLoopMockCLITestSupport.task(
            id: "RQ-B",
            status: .todo,
            title: "Fresh B Task",
            priority: .medium,
        ...
        try CueLoopMockCLITestSupport.writeQueueFile(in: workspaceADir, tasks: [staleTask])
        try CueLoopMockCLITestSupport.writeQueueFile(in: workspaceBDir, tasks: [freshTask])
        ...
        let graphAURL = try WorkspaceRunnerConfigurationTestSupport.writeGraphDocument(
            in: rootDir,
            name: "graph-a.json",
            tasks: [CueLoopMockCLITestSupport.graphNode(id: "RQ-A", title: "Stale Graph A")]
        )
        let graphBURL = try WorkspaceRunnerConfigurationTestSupport.writeGraphDocument(
            in: rootDir,
            name: "graph-b.json",
            tasks: [CueLoopMockCLITestSupport.graphNode(id: "RQ-B", title: "Fresh Graph B")]
        )
        let specAURL = try WorkspaceRunnerConfigurationTestSupport.writeCLISpecDocument(
            in: rootDir,
            name: "cli-spec-a.json",
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: rootDir,
            name: "mock-cueloop-retarget-stale",
            body: script
        )
        let client = try CueLoopCLIClient(executableURL: scriptURL)
        workspace = Workspace(workingDirectoryURL: workspaceADir, client: client)
