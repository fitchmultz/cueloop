...
@MainActor
final class WorkspaceDiagnosticsServiceTests: CueLoopCoreTestCase {
    func testQueueLockDiagnosticSnapshot_usesStructuredUnlockInspectDocument() async throws {
        let fixture = try CueLoopMockCLITestSupport.makeFixture(prefix: "workspace-diagnostics-lock-inspect")
        ...
        workspace = Workspace(workingDirectoryURL: fixture.workspaceURL, client: try CueLoopCLIClient(executableURL: scriptURL))
    ...
    func testQueueValidationOutput_reportsConfiguredQueuePathWhenCustomQueueIsMissing() async throws {
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: fixture.rootURL,
            name: "mock-cueloop-diagnostics-custom-missing",
            body: script
        )
        workspace = Workspace(
        ...
        XCTAssertFalse(output.contains(".cueloop/config.jsonc"))
    ...
    func testQueueValidationOutput_reportsConfigResolutionFailure() async throws {
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: fixture.rootURL,
            name: "mock-cueloop-diagnostics-config-fail",
            body: script
        )
        workspace = Workspace(
            workingDirectoryURL: fixture.workspaceURL,
 ...
 */
@MainActor
final class ContractDiagnosticsPersistenceTests: CueLoopCoreTestCase {
    ...
    func test_persist_createDirectoryFailure_returnsFailureStatus_andTelemetry() throws {
        let directory = try CueLoopCoreTestSupport.makeTemporaryDirectory(prefix: "settings-diagnostics-create-failure")
        defer { CueLoopCoreTestSupport.assertRemoved(directory) }
    ...
    func test_persist_writeFailure_returnsFailureStatus_andTelemetry_forWorkspaceRouting() throws {
        let directory = try CueLoopCoreTestSupport.makeTemporaryDirectory(prefix: "workspace-diagnostics-write-failure")
        defer { CueLoopCoreTestSupport.assertRemoved(directory) }
    ...
    func test_persist_recoversToSuccess_andWritesSuccessOutcome() throws {
        let directory = try CueLoopCoreTestSupport.makeTemporaryDirectory(prefix: "settings-diagnostics-recover-success")
        defer { CueLoopCoreTestSupport.assertRemoved(directory) }
        let diagnosticsURL = directory.appendingPathComponent("settings-diagnostics.json", isDirectory: false)
