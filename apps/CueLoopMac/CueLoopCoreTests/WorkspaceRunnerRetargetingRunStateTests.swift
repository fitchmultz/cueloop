...
final class WorkspaceRunnerRetargetingRunStateTests: WorkspacePerformanceTestCase {
    func test_setWorkingDirectory_invalidatesActiveRunState() async throws {
        ...
        let rootDir = try WorkspacePerformanceTestSupport.makeTempDir(prefix: "cueloop-workspace-retarget-run")
        ...
        try CueLoopMockCLITestSupport.writeQueueFile(in: workspaceADir, tasks: [])
        try CueLoopMockCLITestSupport.writeQueueFile(in: workspaceBDir, tasks: [])
        ...
        let scriptURL = try CueLoopMockCLITestSupport.makeExecutableScript(
            in: rootDir,
            name: "mock-cueloop-retarget-run",
            body: script
        )
